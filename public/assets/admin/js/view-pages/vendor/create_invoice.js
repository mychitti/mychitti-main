...
$("#tnc_id").on("change", function () {
    ...
    $.get({
        url: businessSettingsTncFetchUrl.replace(":id", id),
...
function addMoreRow(item = null) {
    ...
    var html =
        ...
        `" ` +
        readonly +
        ` placeholder="Item Name" class="form-control item_name"></td>
                      <td style="width: 100px;"><label class="small_label">Price</label><input type="number" value="` +
        item_price +
        `"  step="0.1" name="item_price_new[]" placeholder="Price" class="form-control price item_price"></td>
                      <td style="width: 58px;"><label class="small_label">Qty</label><input type="number" name="item_qty_new[]" value="1" placeholder="Qunatity" class="form-control qty item_qty"></td>
                       <td style="width:140px;"><label class="small_label">Unit</label>
                            <select name="item_unit_new[]" class="form-control js-select2-custom unit_select` +
        dataId +
        `">
                      ...
                      <td style="width: 58px;" class="tax_inp_data ` +
        classNew +
        ` tax_field" ><label class="small_label">Tax</label><input value="` +
        tax +
        `" type="number" name="item_tax_new[]" placeholder="Tax" class="form-control tax item_tax"></td>
                      <td style="width: 93px;" class="hsn_inp ` +
        classNew +
        `"><label class="small_label">HSN</label><input value="` +
        item_hsn +
        `" type="text" name="item_hsn_new[]" placeholder="HSN" class="form-control"></td>
        ...
        `"  class="form-control item_total"></td>
                       <td><button type="button"  onclick="deleteNewRow(` +
        dataId +
        `)" class="btn action-btn btn--danger btn-outline-danger"><i class="tio-delete-outlined"></i></button></td>
                    </tr>`;
...
function calculateTotals() {
    ...
    $("#totalWithoutGST").text(totalWithoutGST.toFixed(2));
        ...
        success: function (data) {
            if (data.status) {
                ...
                if (data.action == "add_customer") {
                    ...
                    $(".sign_section").load(
                        url + " .sign_section_inner",
                        function () {
                            $("#sign_id2").select2();
                        },
                    );
                } else if (data.action == "add_bankaccount") {
                    $(".account_section").load(
                        url + " .account_section_inner",
                        function () {
                            $("#sign_id2").select2();
                        },
                    );
                } else if (data.action == "add_inventory") {
                    $(".inventory_section").load(
                        url + " .inventory_section_inner",
                        function () {
                            $(".inv_close_btn").click();
...
$("#inventory_items").on("change", function () {
    ...
    if (
        selectedVal === "add_new" ||
        (Array.isArray(selectedVal) && selectedVal.includes("add_new"))
    ) {
...
function showUserCard(data) {
    ...
    var html =
        ...
        `</p> ` +
        (data.email
            ? `<p class="mb-0" style="font-size:12px">` + data.email + `</p>`
            : "");
    if (data.billing_address) {
        let billing = data.billing_address;
        html +=
            `<h6 class="mb-0 mt-2">Billing Address</h6><p class="mb-0" style="font-size:12px">` +
            (billing.address1 ?? "") +
            " " +
            (billing.address2 ?? "") +
            " " +
    ...
    if (data.shipping_address) {
        ...
        html +=
            `<h6 class="mb-0 mt-2">Shipping Address</h6><p class="mb-0" style="font-size:12px">` +
            (shipping.address1 ?? "") +
            " " +
            (shipping.address2 ?? "") +
            " " +
...
function validateInvoiceNum(elem, tax_type) {
    ...
    $.ajax({
        url: invoiceValidateInvoiceNumUrl,
        type: "POST",
        data: {
            number: $(elem).val(),
            prefixe: prefix,
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Customer selection
    const selectCustomerBtn = document.querySelector(
    ...
    const actionButtons = document.querySelectorAll(
        ".totals-section .btn, .totals-section .btn-primary",
    );
...
$(document).on("change", "#sign_id2", function () {
    ...
    if (selectedVal === "add_new") {
        ...
        $.post({
            url: businessSettingsSignatureFetchUrl,
            data: {
                id: sign_id,
            },
            success: function (data) {
                var html =
                    `<img width="200px" style="margin:5px 0" src="` +
                    data +
                    `" alt="sign">`;
...
$(".tax_rate").on("change", function () {
    ...
    // CESS logic
    toggle($('.tax_rate[value="cess"]').is(":checked"), [
        ".cess_inp",
        ".cess_amount_label",
        ".cess_amount_fld",
    ]);
    ...
    // SURCHARGE logic
    toggle($('.tax_rate[value="surcharge"]').is(":checked"), [
        ".surcharge_inp",
        ".surcharge_amount_fld",
        ".surcharge_amount_label",
    ]);
...
$(document).ready(function () {
    $("#sign_id2").on("select2:open", function () {
        setTimeout(function () {
            const $addNewOption = $(
                'li.select2-results__option[id$="-add_new"]',
            );
            console.log($addNewOption); // Should show the LI in console
    ...
    $("#customer_id, #inventory_items, #tnc_id").on(
        "select2:open",
        function () {
            setTimeout(function () {
                const $addNewOption = $(
...
function recalculateInvoice() {
    ...
    $(".item_row").each(function () {
        ...
        $(this).find(".item_taxable").val(amount.toFixed(2));
        $(this)
            .find(".item_total")
            .val((amount + taxAmount).toFixed(2));
    });

    let add_chrg_html = "";
    $(".add_chrg_row").each(function () {
        const chargeInput =
            parseFloat($(this).find(".additional_charges").val()) || 0;
        ...
        let chargeNameInput = $(this)
            .closest("tr")
            .find('input[name="charges_name[]"]');
        let chargesInput = $(this)
            .closest("tr")
            .find('input[name="add_charges[]"]');
        if (
            $(chargeNameInput).val().trim() !== "" &&
            $(chargesInput).val().trim() !== ""
        ) {
            add_chrg_html += `<div class="col-md-6 tcs-label-small">${$(chargeNameInput).val()}</div>
                              <div class="col-md-6 text-right">${charges.toFixed(2)}</div>`;
        }
    });
    ...
    // Update UI
    $(".taxable_amount_fld").text(taxableAmount.toFixed(2));
    $(".total-tax-field").text(finalTax.toFixed(2));
    $(".tds_amount_fld").text(tdsAmount.toFixed(2));
    $(".tcs_amount_fld").text(tcsAmount.toFixed(2));

    $(".cess_amount_fld").text(cessAmount.toFixed(2));
    $(".surcharge_amount_fld").text(surchargeAmount.toFixed(2));

    $(".tds_gst_amount_fld").text(tdsGstAmount.toFixed(2));
    $(".surcharge_amount_fld").text(surchargeAmount.toFixed(2));
    $(".cess_amount_fld").text(cessAmount.toFixed(2));
    $(".invoice-total-amount").text(totalAmount.toFixed(2));
    $(".total_discount_fld").text(discount.toFixed(2));
...
$("body").on(
    "keyup change",
    ".tax_type, .item_price, .item_tax, .charges_tax_status, .tds_under_gst, .tax_rate, .item_qty, #cess_inp, #surcharge_inp, .additional_charges, .charges_tax, .remove_add_charge, .tds_under_gst, .charges_tax_status, #tcs_rate_id, #tds_rate_id, .total_discount, .total_discount_type ",
    function () {
        recalculateInvoice();
    },
);
...
function checkPricing(elem = null) {
    ...
    $("#selling_price").val(final_price.toFixed(2));
    $("#sell_price_show").text(final_price.toFixed(2));
    $("#taxable_amount").val(base_price.toFixed(2)); // Taxable amount is always before GST
    validateMRP();
