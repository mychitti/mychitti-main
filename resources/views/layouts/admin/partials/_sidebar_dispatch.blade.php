            ...
            <div class="navbar-vertical-content bg-white" id="navbar-vertical-content">
                <form autocomplete="off" class="sidebar--search-form">
                    <div class="search--form-group">
                        ...
                        <input autocomplete="false" type="text" name="qq" class="form-control form--control"
                ...
                <ul class="navbar-nav navbar-nav-lg nav-tabs">
                    <!-- Dashboards -->
                    <li class="navbar-vertical-aside-has-menu {{ Request::is('dispatch') ? 'show active' : '' }}">
                    ...
                    @if (\App\CentralLogics\Helpers::module_permission_check('order'))
                        ...
                        @foreach ($modules as $module)
                            @if ($module->module_type != 'parcel')
                                ...
                                <li
                                    ...
                                    <ul class="js-navbar-vertical-aside-submenu nav nav-sub"
                                        style="{{ Request::is('dispatch*') ? 'display-block' : 'display-none' }}">
                                        <li
                            ...
                            @else
                                @php(
    $unassigned = \App\Models\Order::whereHas('module', function ($query) use ($module) {
        $query->where('module_id', $module->id);
    })->SearchingForDeliveryman()->OrderScheduledIn(30)->ParcelOrder()->count()
)
                                @php(
    $ongoing = \App\Models\Order::whereHas('module', function ($query) use ($module) {
        $query->where('module_id', $module->id);
