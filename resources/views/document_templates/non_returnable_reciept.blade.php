<style>
    ...
    .approve_btn {
        background-color: #ffa984ff;
        color: white;
        text-decoration: none;
        font-size: 14px;
        padding: 12px 40px;
        border: 1px solid #ffa984ff;
    }
...
<h2>Product Receivable Receipt (Non-Returnable)</h2>
...
<div class="ack-box">
    The above-mentioned product(s) are received on a **non-returnable** basis. The customer has permanently submitted
    the items and waives all rights to claim them back.<br><br>
    No responsibility will be taken for return, repair, or future recovery.
</div>

<table class="info-table">
    <tr>
        <td><strong>Received
                By:</strong>{{ isset($data['employee']) && $data['employee'] ? $data['employee']->f_name . ' ' . $data['employee']->l_name : 'N/A' }}
        </td>
        <td>
            <strong>Customer Name:</strong> {{ isset($data['client']) ? $data['client']->f_name : '' }} <br><br>
        </td>
    </tr>
    <tr>
        <td><strong>Signature:</strong> ______________</td>
        <td> <strong>Customer Signature:</strong> ______________</td>
    </tr>
    <tr>
        <td><strong>Date:</strong> {{ date('d/m/Y') }}</td>
        <td align="right">

            @if (!$data['approved']  && isset($data['rr_id']) && $data['rr_id'])
