...
<template id="challanRowTemplate">
    <tr class="item-row">
        <td>
            <select name="items[__IDX__][laundry_item_id]" class="form-control form-control-sm item-select">
                <option value="" data-name="" data-rate="">-- Custom --</option>
                @foreach($items as $item)
                    <option value="{{ $item->id }}" data-name="{{ $item->name }}" data-rate="{{ $item->price }}">
...
<script>
    ...
    // After adding a new vendor via modal, auto-select them in the dropdown
