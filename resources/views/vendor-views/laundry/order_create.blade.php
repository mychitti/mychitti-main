...
<template id="itemRowTemplate">
    <tr class="item-row">
        <td>
            <select name="items[__IDX__][laundry_item_id]" class="form-control form-control-sm item-select" data-placeholder="Select or type...">
                <option value="">-- Custom --</option>
                @foreach($items as $item)
                    <option value="{{ $item->id }}" data-name="{{ $item->name }}" data-price="{{ $item->price }}">
                        {{ $item->name }} ({{ ucfirst($item->category) }}) — {{ number_format($item->price,2) }}
                    </option>
                @endforeach
            </select>
...
<script>
    ...
    // Auto-fill name/phone when a customer is selected from AJAX search
    $(document).on('change', '#customer_id', function () {
        var val = $(this).val();
        if (!val || val === 'add_new') return;
        // Parse text: "Name | type (phone)" or "Name (phone)"
    ...
    // After adding a new customer via modal, auto-select them
