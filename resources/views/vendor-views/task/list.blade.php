                ...
                <div class="d-flex gap-4 py-4 px-2 flex-wrap">
                    ...
                    @foreach ($tasks as $key => $task)
                        ...
                        <div class="repair-card-container {{ $status . '-color' }} "
                            @if (hasPermission('task', 'view')) onclick="handleClick('{{ route('vendor.task.detail', [$task->id]) }}', event)" @endif>
                                    ...
                                    <div class="dropdown-menu">
                                        @if (hasPermission('task', 'view'))
                                            <a class="dropdown-item text-warning"
                                                href="{{ route('vendor.task.detail', [$task->id]) }}"
                                        ...
                                        @if (hasPermission('task', 'edit'))
                                            ...
                                            @if ($task->employee_id == \App\CentralLogics\Helpers::get_loggedin_user()->id || hasPermission('task', 'edit_others'))
                                                <a class="dropdown-item  text--primary"
                                                    href="{{ $task->parent_id ? route('vendor.task.subtask.edit', [$task->id]) : route('vendor.task.edit', [$task->id]) }}"
                                        ...
                                        @if (hasPermission('task', 'delete'))
                                            ...
                                            <form
                                                action="{{ $task->parent_id ? route('vendor.task.subtask.delete', [$task['id']]) : route('vendor.task.delete', [$task['id']]) }}"
