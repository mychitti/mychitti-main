...
@section('content')
    ...
    <header class="animated-header" id="mainHeader">
        <div class="header-container">
            <div class="logo-animated">
                <img loading="lazy" class="logo-img-animated" src="{{ asset('storage/app/public/store/') . '/' . $store['logo'] }}" alt="{{ $store['name'] }}">
    ...
    <div class="hero-parallax">
        ...
        <div class="hero-content-wrapper">
            ...
            <div class="hero-visual-animated">
                <img loading="lazy" class="hero-img-animated" src="{{ asset('storage/app/public/store/cover/') . '/' . $store['cover_photo'] }}" alt="Cover">
                ...
                <div class="content-left animate-on-scroll slide-left">
                    ...
                    <div class="content-text" id="text-{{ $store['id'] }}">
                        ...
                        @if (strlen($description) > 400)
                            ...
                            <a class="cursor-pointer" style="color: #667eea; font-weight: 700; text-decoration: underline;" onclick="toggleReadMore({{ $store['id'] }})"
                    ...
                    <div class="info-card-animated">
                        ...
                        <ul class="contact-list-animated">
                            <li class="contact-item-animated">
                                ...
                                <div>
                                    <h4 style="font-size: 12px; text-transform: uppercase; color: var(--text); margin-bottom: 0.5rem;">Phone</h4>
                                    @php
                            ...
                            <li class="contact-item-animated">
                                ...
                                <div>
                                    <h4 style="font-size: 12px; text-transform: uppercase; color: var(--text); margin-bottom: 0.5rem;">Email</h4>
                                    <p style="font-size: 15px; font-weight: 600; margin: 0;">
                                        <a href="mailto:{{ $store['email'] }}" style="color: var(--dark); text-decoration: none;">{{ $data['store_config']?->webpage_email ?? $store->email }}</a>
                                    </p>
                                </div>
                            </li>
                            <li class="contact-item-animated">
                                ...
                                <div>
                                    <h4 style="font-size: 12px; text-transform: uppercase; color: var(--text); margin-bottom: 0.5rem;">Address</h4>
                                    <p style="font-size: 15px; font-weight: 600; margin: 0;">{{ $store['address'] }}</p>
                                </div>
                            </li>
                        </ul>

                        <div style="margin-top: 2rem; padding: 1.5rem; background: var(--primary-gradient); border-radius: 15px; text-align: center; color: white;">
                            @php $store_rating = number_format($store->average_rating, 1); @endphp
    ...
    <div style="max-width: 1400px; margin: 3rem auto; padding: 0 3rem;">
        <div class="owl-carousel banner-carousel animate-on-scroll fade-in">
            @foreach ($data['banners'] as $value)
                <a href="{{ $value->default_link ?? '#' }}">
                    <img loading="lazy" src="{{ asset('storage/app/public/banner/') . '/' . $value->image }}" alt="banner" style="border-radius: 20px; width: 100%;">
                </a>
            @endforeach
        </div>
    </div>
        ...
        <div class="section-container">
            ...
            @foreach ($productdata as $key => $cat)
                <div style="margin: 4rem 0;">
                    <h3 class="animate-on-scroll slide-left" style="font-size: 2rem; font-weight: 800; margin-bottom: 2.5rem; color: var(--dark);">{{ $cat->name }}</h3>
                    ...
                    <div class="products-grid-animated">
                        @foreach ($cat->items as $index => $pro)
                            ...
                            @endphp
                            <div class="pr_{{ $pro->id }} product-card-animated stagger-item" style="transition-delay: {{ $index * 0.1 }}s;">
                                ...
                                <div class="product-details-animated">
                                    ...
                                    @if ($module == 5)
                                        <p style="font-size: 13px; color: var(--text); margin-bottom: 0.75rem; min-height: 20px;">
                                            {{ !empty($variations) ? $variations[0]->type : '' }}
                                        </p>
                                        @if (count($variations) > 1)
                                            <span style="display: inline-block; background: var(--light); padding: 0.3rem 0.85rem; border-radius: 12px; font-size: 11px; font-weight: 600;">+{{ count($variations) - 1 }} more</span>
                                        @endif

                                        <div style="display: flex; align-items: baseline; gap: 1rem; margin: 1.25rem 0;">
                                            <div style="font-size: 1.75rem; font-weight: 900; color: var(--dark);">{{ _price($selling_price) }}</div>
                                            @if ($pro->discount > 0)
                                                <div style="font-size: 1rem; color: var(--text); text-decoration: line-through;">{{ _price($mrp) }}</div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="cartSec_{{ $pro->id }}">
                                            ...
                                            @if (_itemExistInCart($pro->id, json_encode('[' . $firstVr . ']')))
                                                <button onclick="updateCart({{ $pro->id }}, 'remove','{{ !empty($variations) ? 0 : '' }}',  {{ _itemExistInCart($pro->id, json_encode('[' . $firstVr . ']')) }})"
                                                    class="btn-product-animated" style="background: var(--secondary-gradient);">
                ...
                <div class="gallery-grid-animated">
                    @foreach ($data['galleries'] as $index => $value)
                        <a target="_blank"
                            ...
                            class="gallery-item-animated stagger-item lightgallery-item" style="transition-delay: {{ $index * 0.1 }}s;">
                            <img loading="lazy" 
        ...
        <div class="section-animated" id="reviews" style="background: var(--light);">
            <div class="section-container">
                ...
                @foreach ($data['reviews'] as $index => $rev)
                    <div class="animate-on-scroll" style="background: white; border-radius: 20px; padding: 2.5rem; margin-bottom: 2rem; box-shadow: 0 5px 25px rgba(0,0,0,0.08); {{ $index % 2 === 0 ? 'margin-right: 5rem;' : 'margin-left: 5rem;' }}"
                        ...
                        <div style="display: flex; gap: 1.5rem; margin-bottom: 1.5rem;">
                            <img loading="lazy" 
                                ...
                                alt="{{ $rev->f_name }}">
                            <div style="flex: 1;">
                                <div style="font-size: 1.375rem; font-weight: 800; color: var(--dark); margin-bottom: 0.5rem;">{{ $rev->f_name . ' ' . $rev->l_name }}</div>
                                <div style="font-size: 12px; color: var(--text); margin-bottom: 0.75rem;">{{ _formatted_datetime($rev->created_at) }}</div>
                                <div style="display: flex; gap: 0.25rem;">
                                    @for ($i = 1; $i < 6; $i++)
                                        <i class="fas fa-star" style="font-size: 16px; color: {{ $rev->rating >= $i ? '#fbbf24' : '#e0e0e0' }};"></i>
                                    @endfor
                                </div>
                            </div>
                        </div>
                                ...
                                <div style="display: flex; gap: 1rem; margin-top: 1.5rem;">
                                    @foreach ($attachments as $img)
                                        <a target="_blank" href="{{ \App\CentralLogics\Helpers::onerror_image_helper($img, asset('storage/app/public/') . '/' . $img, asset('public/assets/admin/img/160x160/img1.jpg'), '/') }}">
                                            <img loading="lazy" style="width: 100px; height: 100px; border-radius: 12px; object-fit: cover; border: 2px solid #e2e8f0; transition: transform 0.3s; cursor: pointer;"
                        ...
                        @if ($rev->reply)
                            <div style="margin-top: 2rem; padding: 1.75rem; background: var(--light); border-left: 4px solid #667eea; border-radius: 12px;">
                                <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                                    <img loading="lazy" 
                                        ...
                                        alt="{{ $store->name }}">
                                    <div>
                                        <div style="font-weight: 800; font-size: 15px; color: var(--dark);">Store Response</div>
                                        <div style="font-size: 12px; color: var(--text);">{{ _formatted_datetime($rev->replied_at) }}</div>
                                    </div>
                                </div>
                                <p style="font-size: 14px; color: var(--text); line-height: 1.8; margin: 0;">{{ $rev->reply }}</p>
                            </div>
                        @endif
                    </div>
                @endforeach
                ...
                @if ($data['review_count'] > 2)
                    <div style="text-align: center; margin-top: 3rem;" class="animate-on-scroll fade-in">
                        <a href="{{ route('store.reviews', [$store->slug]) }}" class="btn-primary-animated" style="display: inline-block;">
    ...
    <div class="section-animated" id="contact" style="background: white;">
        <div class="section-container">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2.5rem; margin-top: 3rem;">
                @php $contacts = [
                    ['icon' => 'map', 'title' => 'Our Location', 'value' => $store['address'], 'link' => null],
                    ['icon' => 'secured-letter', 'title' => 'Email Address', 'value' => $store['email'], 'link' => 'mailto:' . $store['email']],
                    ['icon' => 'phone', 'title' => 'Phone Number', 'value' => $store['phone'], 'link' => 'tel:' . $store['phone']],
                    ['icon' => 'marker', 'title' => 'Get Directions', 'value' => 'View on Map', 'link' => '#', 'modal' => true]
                ]; @endphp

                @foreach ($contacts as $index => $contact)
                    <div class="stagger-item" style="transition-delay: {{ $index * 0.15 }}s; background: var(--light); padding: 2.5rem; border-radius: 20px; text-align: center; transition: all 0.3s; cursor: pointer;"
                         onmouseover="this.style.transform='translateY(-10px)'; this.style.boxShadow='0 15px 50px rgba(0,0,0,0.1)';"
                         onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
                        <div style="width: 70px; height: 70px; background: var(--primary-gradient); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.5rem;">
                            <img loading="lazy" src="https://img.icons8.com/ios-filled/50/{{ $contact['icon'] }}.png" alt="{{ $contact['title'] }}" style="width: 35px; height: 35px; filter: brightness(0) invert(1);">
                        </div>
                        <div style="font-size: 1.25rem; font-weight: 800; color: var(--dark); margin-bottom: 0.75rem;">{{ $contact['title'] }}</div>
                        <div style="font-size: 14px; color: var(--text);">
                            @if ($contact['link'])
                                @if (isset($contact['modal']))
                                    <a class="cursor-pointer" data-bs-toggle="modal" data-bs-target="#exampleModal" style="color: #667eea; text-decoration: none; font-weight: 600;">{{ $contact['value'] }}</a>
                                @else
                                    <a href="{{ $contact['link'] }}" style="color: #667eea; text-decoration: none; font-weight: 600;">{{ $contact['value'] }}</a>
                                @endif
                            @else
                                {{ $contact['value'] }}
                            @endif
    ...
    <div class="section-animated" id="about" style="background: var(--light);">
        <div class="section-container">
            ...
            <div class="animate-on-scroll fade-in" style="background: white; border-radius: 20px; padding: 3.5rem; box-shadow: 0 5px 25px rgba(0,0,0,0.08); line-height: 2; font-size: 15px; color: var(--text);">
