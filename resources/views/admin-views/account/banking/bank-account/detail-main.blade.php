...
@section('title', 'Main Account Detail')
...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="page-header d-flex flex-wrap justify-content-between align-items-center w-100">
            <div>
                <h1 class="page-header-title"><i class="tio-filter-list"></i> Main Account Detail</h1>
            </div>
            ...
            <div class="bank-header shadow">
                <div class="bank-account-info">
                    <div class="bank-account-details">
                        <h2>
                            {{ \App\Models\BusinessSetting::where('key', 'business_name')->first()?->value ?? '' }} Bank Statement
                        </h2>
                        <div class="d-flex gap-2 flex-wrap">
                            @foreach ($accounts as $key => $value)
                                @php
                                    $baseDir = storage_path('app/public/bank_logos');
                                    $folderName = basename($value?->bank_name);
                                    $logoUrl = asset('storage/app/public/bank_logos/' . $folderName . '/logo.png');
                                @endphp
                                <a href="{{ route('admin.account.banking.bank-account.detail', [$value?->id]) }}"
                                    class="border rounded p-2 d-flex flex-column justify-content-between">
                                    <img src="{{ $logoUrl }}" style="width:120px;margin: 3px 0 ;" alt=""><br>
            ...
            <div class="bank-main-content">
                <div class="bank-transactions-header">
                    ...
                    <div class="bank-filters">
                        ...
                        <a href="{{ route('admin.account.banking.bank-account.transaction-export', ['all']) }}"
                            class="bank-action-btn bank-secondary">Export Transactions</a>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach ($transactions as $key => $txn)
                                @php
                                    $type = $txn->type;
                                @endphp
                                <tr class="bank-transaction-row" data-type="{{ $type }}" data-status="completed">
                                    ...
                                    <td class="bank-td-date">
                                        @php
                                            $baseDir = storage_path('app/public/bank_logos');
                                            $folderName = basename($txn->bankAccount?->bank_name);
                                            $logoUrl = asset(
                                                'storage/app/public/bank_logos/' . $folderName . '/logo.png',
                                            );
                                        @endphp
                                        <img src="{{ $logoUrl }}" width="80" style="margin: 3px 0 ;"
                                            alt=""><br>
                                    </td>
                                    <td class="bank-transaction-title">{{ $txn->txn_id }}</td>
                                  ...
                                  @if ($type == 'debit')
                                        <td class="bank-td-amount bank-{{ $type }}">
                                            <span class="text-center">{{ $type == 'credit' ? '+' : '-' }}{{ _price($txn->amount) }}</span>
                                        </td>
                                    @else
                                        <td></td>
                                    @endif
    ...
    @include('admin-views.form_modals.financial_year_select')
