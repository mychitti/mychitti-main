 ...
 @section('content')
     <div class="content container-fluid">
             ...
             <div class="d-flex flex-wrap justify-content-between align-items-center">
                 ...
                 <div class="d-flex align-items-center">
                     ...
                     <div class="d-flex gap-2">
                         ...
                         {{-- // export --}}
                         @if (hasPermission('reports_tax_report', 'export'))
                             <a href="{{ route('admin.account.report.tax', ['export']) }}"
         ...
         <!-- Page Heading -->
         @if (hasPermission('reports_tax_report', 'list'))
             <div class="card">
                 <div class="card-header d-flex flex-column align-items-start">
                     ...
                     <div class="totals">
                         <div class="total-box credit">
                             <div class="total-label">Purchase</div>
                             <div class="total-value">Total Purchase Amount :
                                 {{ _price($purchaseInvoices->map(fn($i) => $i->taxable_amount ?? $i->total_amount)->sum()) }}
                             </div>
                             <div class="total-value">Total GST Amount : {{ _price($purchaseInvoices->sum('final_tax')) }}
                             </div>
                         </div>
                         <div class="total-box credit">
                             <div class="total-label">Sale</div>
                             <div class="total-value">Total Sale Amount :
                                 {{ _price($saleInvoices->map(fn($i) => $i->taxable_amount ?? $i->total_amount)->sum() + $serviceInvoices->map(fn($i) => $i->taxable_amount ?? $i->total_amount)->sum()) }}
                             </div>
                             <div class="total-value">Total GST Amount :
                                 {{ _price($saleInvoices->sum('final_tax') + $serviceInvoices->sum('final_tax')) }}</div>
                         </div>
                         <div class="total-box credit">
                             <div class="total-label">Present Month GST Value</div>
                             <div class="total-value">
                                 {{ _price($saleInvoices->sum('final_tax') + $serviceInvoices->sum('final_tax') - $purchaseInvoices->sum('final_tax')) }}
                             </div>
                         </div>
                     </div>
                 </div>
                 ...
                 <div class="card-body p-0">
                    ...
                    "paging":false
                             ...
                             <tbody id="set-rows">
                                 @foreach ($invoices as $k => $e)
                                     <tr>
                                         <td>{{ $k + 1 }}</td>
                                         <td>{{ $e['invoice_date'] ?? date('Y-m-d', strtotime($e['created_at'])) }}</td>
                                         <td>
                                             @if ($e->pdf)
                                         ...
                                         <td>
                                             @php
                                                 ...
                                                 if ($e['invoice_type'] === 'Service' && $e['websiteUser']) {
                                                     ...
                                                     $profileLink = route('admin.client.view', [
                                                         $e['storeCustomer']->id,
                                                     ]);
                                                 } elseif ($e['seller']) {
                                                     $name = trim($e['seller']?->f_name . ' ' . $e['seller']?->l_name);
                                                     $profileLink = route('admin.client.view', [$e['seller']->id]);
                                                 }
                                             @endphp
