      ...
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body">
                      ...
                      @if (Route::currentRouteName() == 'admin.billing.settings' ||
                           Route::currentRouteName() == 'admin.billing.create-invoice')
                          <form class="customer_add_form" enctype="multipart/form-data" action="{{ route('admin.business-settings.signature.save') }}"
                      method="post">
                      @csrf
                          <input type="hidden" name="type" value="invoice">
                      @else
                        <form class="customer_add_form" enctype="multipart/form-data" action="{{ route('admin.quotation.signature.save') }}"
                      method="post">
                      @csrf
                          ...
                          <div class=" ">
                              <div class="flex-grow-1 mx-auto">
                                  ...
                                  <label class="d-inline-block m-0 position-relative">
                                      @php($img = \App\Models\BusinessSetting::where('key', 'admin_signature')->first())
                                      @php($img = $img->value ?? '')
                                      <img class="img--136 border onerror-image" id="viewer"
                                          src="{{ \App\CentralLogics\Helpers::onerror_image_helper(
                                              $img,
                                              asset('storage/app/public/business') . '/' . $img,
