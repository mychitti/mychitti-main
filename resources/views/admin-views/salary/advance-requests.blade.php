        ...
        <div class="page-header">
            <div class="d-flex flex-wrap justify-content-between align-items-center">
                ...
                <button class="btn btn--primary" data-toggle="modal"
                    data-target="#advanceSalaryModal">{{ auth('admin')->user()->role_id == 1 ? 'Create' : 'Request' }} Advance
                    Payment</button>
            </div>
        </div>
            ...
            <div>
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <a href="{{ route('admin.salary-history') }}" class="nav-link ">Salary History</a>
                    </li>
                    <li class="nav-item" role="presentation">
                        <a href="{{ route('admin.advance-payment') }}" class="nav-link active">Advance History</a>
                    </li>
                </ul>
            </div>
        @endif
                                ...
                                @if(auth('admin')->user()->role_id == 1)
                                    ...
                                    <td>
                                        ...
                                        @if($e->status == 'pending')
                                            ...
                                            <form
                                                action="{{ route('admin.salary.reject-advance', [$e['id']]) }}"
                                                method="get" id="category-{{ $e['id'] }}">
                                                @csrf @method('get')
                                            </form>
                                            <form
                                                action="{{ route('admin.salary.approve-advance', [$e['id']]) }}"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ auth('admin')->user()->role_id == 1 ? 'Create' : 'Request' }} Advance Payment</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form method='post' class="row" action="{{ route('admin.salary.advance-request.store') }}">
                        @csrf
                        @if (auth('admin')->user()->role_id == 1)
