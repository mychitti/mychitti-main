...
<div class="content container-fluid">
    ...
    <div class="page-header">
        <h1 class="page-header-title"><i class="tio-filter-list"></i> Leave Manage </h1>
...
">
                            ...
                            <form action="" class="d-flex align-items-end">
                                <div class="col-4">
                                    ...
                                    <select name="year" style="width: 100%;" class="form-control">
                                        @for($m = 2000; $m <= 2030; $m++) <option value="{{$m}}" {{$filter_year == $m ? 'selected' : '';}}>{{$m}}</option>
                                            @endfor
                                    </select>
                                </div>
                                <div class="col-4">
                                    ...
                                    <select name="month" style="width: 100%;" class="form-control">
                                        ...
                                        @for($m = 1; $m <=12; $m++) @php $month=date('F', mktime(0,0,0,$m, 1, date('Y')));@endphp <option value="{{$m}}" {{$filter_month == $m ? 'selected' : '';}}>{{$month}}</option>
                                            @endfor

                                    </select>
                                </div>
                                <button class=" col-4 btn btn--primary btn-outline-primary">View</button>
                            </form>
                        </div>
                        <div class=" shadow shadow-sm m-2 calendar p-3" style="background-image: linear-gradient(45deg, #ffffff, #fee4a08c);
}">
                            ...
                            <ul class="day-list">
                                ...
                                @for ($d=1; $d < $firstDayOfMonth ; $d++) <li class="day-list-item invisible-item">
                                    </li>
                                    @endfor

                                    @for ($t=1; $t <= $days_in_month ; $t++) <li class="day-list-item" style="cursor:pointer;">{{$t}}
                    ...
                    <div class=" d-flex flex-column p-3" style="width: 50%;">
                    <div class="border rounded my-2 p-2">
                            <h4>Leave Balance of {{date("F", mktime(0, 0, 0, $filter_month, 10)) . ' ' . $filter_year}}</h4> 
                            <table class="table">
                                <tr>
                                    <td>Casual Leaves</td>
                        ...
                        <select name="month" style="width: 100%;" class="form-control leave_type">
                        ...
                        <button class="btn  btn--primary btn-outline-primary my-2" type="button" id="addBtn" style="height: 44px;109px;">Add Leave</button>
                    </div>


                </div>
                 <div class="card-body"> 
                 <h1>Leave History of {{date("F", mktime(0, 0, 0, $filter_month, 10)) . ' ' . $filter_year}}</h1>
        ...
        <script>
            ...
            $('#addBtn').on('click', function() {
                ...
                if (slectedDay == 0) {
                    ...
                    $.post({
                        url: '{{route('admin.users.leave.save')}}',
                        data: {
                            day: day,
                            month: month,
                            year: year,
            ...
            $("#att_form").on('submit', function(e) {
                ...
                $.post({
                    url: '{{route('admin.users.attendance.save')}}',
                    data: {
                        statusArr: statusArr,
                        daysArr: daysArr,
                        year: year,
