...
@push('css_or_js')
    ...
    <style>
        ...
        .tasks-area {
            {{-- min-height: 200px; --}} {{-- padding: 16px; --}} border: 2px dashed transparent;
            border-radius: 8px;
            transition: all 0.2s;
        }
...
@section('content')
    ...
    @if (hasPermission($task_type, 'view'))
        <div class="">
            <!-- Page Header -->
            <div class="page-header d-flex w-100 justify-content-between flex-wrap">
                <div class="d-flex flex-wrap">
                    ...
                    <div class="ml-2">
                        @if (
                            !$task_closed &&
                                hasPermission($task_type, 'edit') &&
                                ($task->employee_id == \App\CentralLogics\Helpers::get_loggedin_user()->id ||
                                    hasPermission('task', 'edit_others')))
                            <a href="{{ $task->parent_id ? route('admin.task.subtask.edit', [$task->id]) : route('admin.task.edit', [$task->id]) }}"
            ...
            <div class="row align-items-start g-0">
                <div class="col-md-8 pr-0">
                    ...
                    @else
                        <form action="{{ route('admin.quotation.save-info-task', [$task->id]) }}" method="post">
                            @csrf
                            @include('admin-views/form_modals/quote_add_modal')
                        </form>
                    @endif
                    @php $existingItems = $data['existing_jobcard_items']; @endphp
                    @include('admin-views/form_modals/basic_invoice')
                    ...
                    <div class="row g-0">
                            ...
                            <div class="card border shadow-sm h-100">
                                <div class="card-body p-3">
                                    ...
                                    <div class="d-flex gap-2 flex-wrap">
                                        ...
                                        @endif
                                        @if (hasPermission($task_type, 'receivable_receipt'))
                                            ...
                                            @endif
                                        @endif
                                        @if (hasPermission($task_type, 'quotation'))
                                            @if ($data['quotation'] && $data['quotation']->pdf)
                                            ...
                                            @endif
                                        @endif
                                        @if (hasPermission($task_type, 'invoice') )
                        ...
                        <div class="col-md-6 p-2">
                            ...
                            <div class="timeline card border">
                                <button data-toggle="modal" data-target="#timelineModal"
                        ...
                        <div class="col-md-6 p-2">
                            <div class="timeline border card ">
                                <div class="d-flex justify-content-between">
                                    <h3>Subtasks</h3>
                                    @if (!$task_closed && hasPermission('subtask', 'add'))
                                ...
                                <div id="tasksArea" class="tasks-area">
                                    @if (hasPermission('subtask', 'list'))
                                        {{-- <span  class="text-center">{{ !count($task->children) ? 'No Tasks Yet... ' : '' }}</span> --}} <div id="tasksContainer">
                                        </div>
                                        @if (!$task_closed && hasPermission('subtask', 'add'))
                ...
                <div class="col-md-4 ">
                                ...
                                @if (hasPermission($task_type, 'status_change') &&
                                    ...
                                    @if (!$isRunning && !$isPaused)
                                        <button type="button" class="btn btn-sm btn-outline-success start_job_btn"
                                            ...
                                            data-job-action="start" data-url="{{ route('admin.task.job-otp-verify') }}"
                                    ...
                                    @else
                                        <button type="button" class="btn btn-sm btn-outline-danger start_job_btn"
                                            ...
                                            data-url="{{ route('admin.task.job-otp-verify') }}"
                                            data-task="{{ $task->id }}" data-phone="{{ $task->user?->phone }}">
                                            End Job <i class="tio-stop-circle-outlined"></i>
                                        </button>
                                        @if ($isRunning)
                                            <button type="button" class="btn btn-sm btn-outline-warning quick-status-btn"
                                                data-url="{{ $task->parent_id ? route('admin.task.subtask.status.update') : route('admin.task.status.update') }}"
                    ...
                    <div class=" card border shadow-sm h-100">
                        ...
                        @endif --}}
                                    @if (!$task_closed && hasPermission($task_type . '_update', 'add'))
                        ...
                        <div class="card-body p-3">
                                                    ...
                                                    <div class="collapse" id="collapseExample{{ $key }}">
                                                        <div class="border-top pt-3 mt-2">
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="d-flex gap-2">
                                                                    @if (!$task_closed && hasPermission($task_type . '_update', 'edit'))
                                                                        <a href="{{ $task->parent_id ? route('admin.task.subtask-update.edit', [$comment['id']]) : route('admin.task.comment.edit', [$comment['id']]) }}"
                                                                            class="btn btn-sm btn-outline-primary">
                                                                            <i class="tio-edit"></i>
                                                                        </a>
                                                                    @endif
                                                                    @if (!$task_closed && hasPermission($task_type . '_update', 'delete'))
                                                                        <a href="{{ $task->parent_id ? route('admin.task.subtask-update.delete', [$comment['id']]) : route('admin.task.comment.delete', [$comment['id']]) }}"
                            ...
                            @else
                                <div class="text-center py-5">
                                    ...
                                    <p class="text-muted mt-3 mb-0">No work updates yet</p>
                                    @if (!$task_closed && hasPermission($task_type . '_update', 'add'))
                                        <button data-toggle="modal" data-target="#addTaskCommentModal"
        ...
        @if (hasPermission('subtask', 'add'))
            <div class="modal fade" id="addSubtaskModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add Subtaskww</h5>
                        ...
                        <div class="modal-body">
                            <div class="add-task-section">
                                <form method="post" id="add_task_form" action="{{ route('admin.task.subtask.add') }}">
                                    @csrf
        ...
        @endif
        @if (hasPermission($task_type . '_update', 'add'))
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="customer_add_form" enctype="multipart/form-data" class="w-100"
                                action="{{ $task->parent_id ? route('admin.task.subtask-update.add') : route('admin.task.comment.add') }}"
                                method="post">
                                @csrf
            ...
            <div class="modal fade" id="taskStartJobOtpModal" tabindex="-1" aria-labelledby="taskStartJobOtpModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="taskOtpForm" action="{{ route('admin.task.job-otp-verify') }}" method="post">
                                @csrf
                        ...
                        <div class="modal-body">
                            <div class="row align-items-end">
                                <form class="customer_add_form status_update_form" enctype="multipart/form-data"
                                    class="w-100"
                                    action="{{ $task->parent_id ? route('admin.task.subtask.status.update') : route('admin.task.status.update') }}"
                                    method="post">
                                    @csrf
        ...
        @if (hasPermission($task_type, 'jobcard'))
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body p-0"> 
                        <form class="w-100 p-0" id="ck_editor_form" enctype="multipart/form-data"
                            action="{{ route('admin.documents.service-report.store-lead', ['save', $task->id]) }}"
                            method="post">
                            @csrf
                            <input type="hidden" name="task_id" value="{{ $task->id }}">
                            <input type="hidden" name="customer" value="{{ $task->client_id }}">
                ...
                <div class="modal-dialog modal-dialog-scrollable modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-1">
                            <form class="w-100 p-0" id="task_form" enctype="multipart/form-data"
                                action="{{ route('admin.documents.receivable-receipt.store-lead', ['save', $task->id]) }}"
                                method="post">
                                @csrf
                                ...
                                <input type="hidden" name="customer" value="{{ $task->client_id }}">
                                <input type="hidden" name="employee_id" value="{{ $task->employee_id }}">
                                @include('admin-views.forms.receivable_receipt_add')
                                <div class="modal-footer">
                    ...
                    @if (!$task_closed)
                        <div class="d-flex align-items-center flex-wrap gap-2">
                            ...
                            @if (_isEnabled('task_invoice'))
                                ...
                                @if (hasPermission($task_type, 'invoice') && $task->client_id)
                                    @if ($data['invoice'] && $data['invoice']->pdf)
                                    
                                    @else

                                       <button class="btn btn--primary btn_sm px-3 py-2 add_attachment"
                                            data-toggle="modal" data-target="#addInvoiceModal">
...
@push('script_2')
    ...
    <tr class="item_row row_${dataId}" data-id="${dataId}">
        ...
        function handleTaskDrop(e, targetTask) {
            ...
            $.get({
                url: "{{ route('admin.task.subtask.update-level') }}",
                data: {
                    parent_id: parent_id,
                    child_id: child_id
                },
        ...
        function createTaskElement(task, level) {
            ...
            const taskDetailUrl = "{{ route('admin.task.subtask.detail', [':id']) }}";
            ...
            @if (hasPermission('subtask', 'delete'))
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('{{ route('admin.task.subtask.getTasks', $task->id) }}')
                .then(response => response.json())
                .then(data => {
                    tasks = data;
                    renderTasks();
    ...
    <script>
        $(document).ready(function() {
            ...
            function afUpdateProgress(value, save = false) {
                ...
                if (save) {
                    $.ajax({
                        url: '{{ route('admin.task.save-progress') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            task_id: $taskId,
            ...
            function updateCountdown() {
                ...
                if (diff <= 0) {
                    ...
                    countdownEl.innerHTML =
                        `<h4 class="text-danger">The deadline has been crossed.</h4><small>Exceeded by</small> <br><span class="time_text">${formatTime(diff)}</span>`;
                } else {
    ...
    @include('admin-views/js/jobcard-js');
    @include('admin-views/multiple_ck_editor');
@endpush
