 <script>
     ...
     function fetch_variation_dets(vr_id, rowId) {
         ...
         $.ajax({
             url: '{{ route('admin.inventory.get_variation_details') }}',
             type: "POST",
             data: {
                 _token: $('[name="_token"]').val(),
                 item_id: item_id,
                    ...
                    <select name="item_id[]"
                        ...
                        @foreach ($items as $item)
                            <option value="{{ $item['id'] }}">
                                {{ str_replace('`', '&#96;', $item['item_name'] . ' | ' . ($item['company_sku_id'] ?? $item['sku_id']) . ' | ' . $item['model_number']) }}
                            </option>
                        @endforeach
                    </select>
                </td>
         ...
         $.post({
             ...
             success: function(data) {
                 ...
                 if (data.status) {
                     ...
                     setTimeout(function() {
                         location.href = '{{ route('admin.inventory.index') }}';
                     }, 1000);
                 }else{
...
<div class="col-xl-4 col-lg-6">
    <div class="row g-2">
     ...
     function combination_update() {
         ...
         $.ajax({
             type: "POST",
             url: '{{ route('admin.inventory.item.variant-combination') }}',
