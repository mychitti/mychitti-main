            ...
            <div class="card-header py-2 border-0">
                <div class="search--button-wrapper justify-content-end">
                    <h5 class="card-title mr-auto">
                        {{ translate('messages.deliveryman_list') }}<span class="badge badge-soft-dark ml-2"
                    ...
                    <div class="min--200">
                        <select name="filter" class="form-control js-select2-custom set-filter" data-filter="filter"
                            ...
                            <option {{ request()?->get('filter') == 'active' ? 'selected' : '' }} value="active">
                                {{ translate('messages.Online') }}</option>
                            <option {{ request()?->get('filter') == 'inactive' ? 'selected' : '' }} value="inactive">
                                {{ translate('messages.Offline') }}</option>
                            <option {{ request()?->get('filter') == 'blocked' ? 'selected' : '' }} value="blocked">
                                {{ translate('messages.Suspended') }}</option>
                        </select>
                    </div>
                    <div class="min--200">
                        <select name="job_type" class="form-control js-select2-custom set-filter" data-filter="job_type"
                            ...
                            <option {{ request()?->get('job_type') == 'freelancer' ? 'selected' : '' }} value="freelancer">
                                {{ translate('messages.Freelancer') }}</option>
                            <option {{ request()?->get('job_type') == 'salary_base' ? 'selected' : '' }}
                                value="salary_base">{{ translate('messages.Salary_Base') }}</option>
                        </select>
                    </div>
                    ...
                    <form class="search-form">
                        <div class="input-group input--group">
                            <input id="datatableSearch_" type="search" name="search" class="form-control h--45px"
                                placeholder="{{ translate('ex:_DM_name_email_or_phone') }}"
                                value="{{ request()->get('search') }}" aria-label="Search" required>
                        ...
                        @foreach ($deliveryMen as $key => $dm)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <label
                                            class="text--title font-medium mb-0">{{ translate('messages.zone_deleted') }}</label>
                                    @endif
                                </td>
                                <td>
                                    <a class="deco-none"
                                        href="{{ route('admin.users.delivery-man.preview', ['id' => $dm['id'], 'tab' => 'transaction']) }}">{{ count($dm['order_transaction']) }}</a>
                                </td>
                                <td>
                                    <div>
                                ...
                                <td>
                                    @if ($dm->status == 1)
                                        <strong
                                            class="text-capitalize text-primary">{{ translate('messages.Active') }}</strong>
                                    @else
                                        <strong
                                            class="text-capitalize text-danger">{{ translate('messages.Suspended') }}</strong>
                                    @endif
                                ...
                                <td>
                                    <div class="btn--container justify-content-center">
                                        ...
                                        <a class="btn action-btn btn--danger btn-outline-danger form-alert"
                                            href="javascript:" data-id="delivery-man-{{ $dm['id'] }}"
