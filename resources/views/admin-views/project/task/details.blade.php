                ...
                <div class="d-flex flex-wrap">
                    ...
                    <div class="ml-2">
                        @if (
                            ...
                            <a href="{{ $task->parent_id ? route('admin.project.task.subtask.edit', [$task->id]) : route('admin.project.task.edit', [$task->id]) }}"
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="d-flex gap-2">
                                                                    @if (!$task_closed && hasPermission('project_'. $task_type . '_update', 'edit'))
                                                                        <a href="{{ $task->parent_id ? route('admin.project.task.subtask-update.edit', [$comment['id']]) : route('admin.project.project.task.comment.edit', [$comment['id']]) }}"
                                                                            class="btn btn-sm btn-outline-primary">
                                                                            <i class="tio-edit"></i>
                                                                        </a>
                                                                    @endif
                                                                    @if (!$task_closed && hasPermission('project_' . $task_type . '_update', 'delete'))
                                                                        <a href="{{ $task->parent_id ? route('admin.project.task.subtask-update.delete', [$comment['id']]) : route('admin.project.task.comment.delete', [$comment['id']]) }}"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="add-task-section">
                                <form method="post" id="add_task_form" action="{{ route('admin.project.task.subtask.add') }}">
                                    @csrf
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="customer_add_form" enctype="multipart/form-data" class="w-100"
                                action="{{ $task->parent_id ? route('admin.project.task.subtask-update.add') : route('admin.project.task.comment.add') }}"
                                method="post">
                                @csrf
                        ...
                        <div class="modal-body">
                            <div class="row align-items-end">
                                <form class="customer_add_form status_update_form" enctype="multipart/form-data"
                                    class="w-100"
                                    action="{{ $task->parent_id ? route('admin.project.task.subtask.status.update') : route('admin.project.task.status.update') }}"
                                    method="post">
                                    @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('admin.project.task.edit', [$task->id]) }}"
                            class="btn btn-outline-primary action-btn"><i class="tio-edit"></i></a>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <a href="{{ route('admin.project.task.edit', [$task->id]) }}" class="btn btn-primary">Edit</a>
                </div>
            </div>
        </div>
    </div>
...
@push('script_2')
    ...
    <tr class="item_row row_${dataId}" data-id="${dataId}">
        ...
        function handleTaskDrop(e, targetTask) {
            ...
            $.get({
                url: "{{ route('admin.project.task.subtask.update-level') }}",
                data: {
                    parent_id: parent_id,
                    child_id: child_id
                },
        ...
        function createTaskElement(task, level) {
            ...
            const taskDetailUrl = "{{ route('admin.project.task.subtask.detail', [':id']) }}";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('{{ route('admin.project.task.subtask.getTasks', $task->id) }}')
                .then(response => response.json())
                .then(data => {
                    tasks = data;
                    renderTasks();
    ...
    <script>
        $(document).ready(function() {
            ...
            function afUpdateProgress(value, save = false) {
                ...
                if (save) {
                    $.ajax({
                        url: '{{ route('admin.project.task.save-progress') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            task_id: $taskId,
