    ...
    <div class="content container-fluid">
        ...
        <div class="card-header d-flex justify-content-end gap-2 flex-wrap py-2">
            ...
            @if (hasPermission('project_task', 'add'))
                <a href="{{ route('admin.project.task.add') }}" class="btn btn_sm btn-primary">+ Add New Task</a>
            @endif
        </div>
                                    ...
                                    <div class="dropdown-menu">
                                        @if (hasPermission('project_task', 'view'))
                                            <a class="dropdown-item text-warning"
                                                href="{{ route('admin.project.task.detail', [$task->id]) }}"
                                        ...
                                        @if (hasPermission('project_task', 'edit'))
                                            ...
                                            @if ($task->employee_id == \App\CentralLogics\Helpers::get_loggedin_user()->id || hasPermission('project_task', 'edit_others'))
                                                <a class="dropdown-item  text--primary"
                                                    href="{{ $task->parent_id ? route('admin.project.task.subtask.edit', [$task->id]) : route('admin.project.task.edit', [$task->id]) }}"
                                        ...
                                        @if (hasPermission('project_task', 'delete'))
                                            ...
                                            <form
                                                action="{{ $task->parent_id ? route('admin.project.task.subtask.delete', [$task['id']]) : route('admin.project.task.delete', [$task['id']]) }}"
