 <form class="client_add_form" enctype="multipart/form-data" class="w-100"
    action="{{ isset($client) ? route('admin.client.update') : route('admin.client.save') }}" method="post">
    @csrf
    <style>
        .addr_btn:hover {
            scale: 1.1;
    ...
    <input type="hidden" id="" name="client_id" value="{{ isset($client) ? $client->id : '' }}">
        ...
        <div class="g-0 row">
            <div class="col-md-6  p-1 ">
                <div class="form-group mb-0">
                    <label class="fomr-label" for="exampleFormControlInput1">Name<span class="text-danger">*</span></label>
                    <input type="text" name="f_name" class="form-control __form-control"
                        placeholder="{{ translate('messages.name') }}" required
                        value="{{ isset($client) ? $client->f_name . ' ' . $client->l_name : '' }}">
                </div>
            </div>
            ...
            <div class="col-md-6  p-1  mb-0">
                ...
                <input type="number" class="form-control iti__tel-input" style="width: 100%;" required name="phone"
                    id="phoneInp" value="{{ isset($client) ? $client->phone : '' }}" placeholder="Ex: 9988776655">
            </div>
                ...
                <div class="col-12 g-0 row p-0">
                    <div class="col-12 p-1" id="custom-buttons2">
                        ...
                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-2 custom-header-btn"
                            style="{{ !isset($client) || !$client->email ? '' : 'display:none;' }}"
                            data-label="email_inp_grp">+ Email</button>

                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-2 custom-header-btn"
                            style="{{ !isset($client) || !$client->gst ? '' : 'display:none;' }}"
                            data-label="gst_inp_grp">+ GST No.</button>

                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-2 custom-header-btn"
                            style="{{ !isset($client) || !$client->id_number ? '' : 'display:none;' }}"
                            data-label="id_inp_grp">+ ID No.</button>

                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-2 custom-header-btn"
                            style="{{ !isset($client) || !$client->id_proof ? '' : 'display:none;' }}"
                            data-label="id_doc_inp_grp">+ ID Proof</button>

                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-2 custom-header-btn"
                            style="{{ !isset($client) || !$client->profile_pic ? '' : 'display:none;' }}"
                            data-label="pp_inp_grp">+ Profile Photo</button>

                    </div>

                    <div class="col-md-6 p-1 hidden_inp pp_inp_grp"
                        style="{{ isset($client) && $client->profile_pic ? '' : 'display: none;' }}">
                    ...
                    <div class="col-md-6  p-1  mb-0 hidden_inp gst_inp_grp"
                        ...
                        <div class="d-flex">
                            <input type="text" class="form-control inp_gst_inp_grp"
                                value="{{ isset($client) ? $client->gst : '' }}" style="width: 100%;"
                    ...
                    <div class="col-md-6  p-1  mb-0 hidden_inp id_inp_grp"
                        ...
                        <input type="text" class="form-control inp_id_inp_grp"
                            value="{{ isset($client) ? $client->id_number : '' }}" style="width: 100%;"
                    ...
                    <div class="col-md-12 p-1 mb-0">
                        <div id="billing_address_remove" class=" bg-white" style="display:none;">
                            ...
                            <div id="billing_address_show">
                                @if (isset($client) && $client->billing_address)
                                    <b> Address Line 1 </b> {{ $client->billing_address->address1 }} <br> <b> Address
                                        Line 2 </b>{{ $client->billing_address->address2 }}<br> <b> State
                                    </b>{{ $client->billing_address->state }}<br> <b> City
                                    </b>{{ $client->billing_address->city }}<br> <b> Pin Code
                                    </b>{{ $client->billing_address->pincode }}
                                @endif
                            </div>
                        </div>
                        ...
                        <div id="billing_address_add">
                            <a class="btn btn-outline-white text-danger fw-bold bg-white addr_btn"
                                style="{{ isset($client) && $client->billing_address ? 'display:none;' : '' }}"
                            ...
                            <div class="collapse {{isset($client) ? 'show' : ''}}" id="collapseBillingAddr">
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="ba_address1"
                                                value="{{ isset($client) && $client->billing_address ? $client->billing_address->address1 : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="ba_address2"
                                                value="{{ isset($client) && $client->billing_address ? $client->billing_address->address2 : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="ba_pincode"
                                                value="{{ isset($client) && $client->billing_address ? $client->billing_address->pincode : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="ba_city" name="ba_city"
                                                value="{{ isset($client) && $client->billing_address ? $client->billing_address->city : '' }}"
                                                placeholder="City">
                                        </div>
                                            ...
                                            <select class="form-control  js-select2-custom" id="ba_state"
                                                ...
                                                @foreach (_states() as $key => $state)
                                                    <option value="{{ $state->id }}"
                                                        {{ isset($client) && $client->billing_address && $client->billing_address->state == $state->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12 p-1 mb-0">
                        <div id="shipping_address_remove" class="bg-white" style="display:none;">
                            ...
                            <div id="shipping_address_show">
                                @if (isset($client) && $client->shipping_address)
                                    <b> Address Line 1 </b> {{ $client->shipping_address->address1 }} <br> <b>
                                        Address Line 2 </b>{{ $client->shipping_address->address2 }}<br> <b> State
                                    </b>{{ $client->shipping_address->state }}<br> <b> City
                                    </b>{{ $client->shipping_address->city }}<br> <b> Pin Code
                                    </b>{{ $client->shipping_address->pincode }}
                                @endif
                            </div>
                        </div>
                        <div id="shipping_address_add">
                            <a class="btn btn-outline-white text-danger fw-bold bg-white addr_btn"
                                style="{{ isset($client) && $client->shipping_address ? 'display:none;' : '' }}"
                            ...
                            <div class="collapse {{isset($client) ? 'show' : ''}}" id="collapseShippingAddr">
                                <div class="card card-body">
                                    ...
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="sa_address1"
                                                value="{{ isset($client) && $client->shipping_address ? $client->shipping_address->address1 : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="sa_address2"
                                                value="{{ isset($client) && $client->shipping_address ? $client->shipping_address->address2 : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="sa_pincode"
                                                value="{{ isset($client) && $client->shipping_address ? $client->shipping_address->pincode : '' }}"
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="sa_city" name="sa_city"
                                                value="{{ isset($client) && $client->shipping_address ? $client->shipping_address->city : '' }}"
                                                placeholder="City">
                                        </div>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <select class="form-control  js-select2-custom" id="sa_state"
                                                ...
                                                @foreach (_states() as $key => $state)
                                                    <option value="{{ $state->id }}"
                                                        {{ isset($client) && $client->shipping_address && $client->shipping_address->state == $state->id ? 'selected' : '' }}>
                                                        {{ $state->state_name . ' (' . $state->state_abbr . ')' }}
                                                    </option>
                                                @endforeach
                                            </select>
            ...
            <div class="form-row col-md-12  p-1 ">
                <div class="col d-flex w-100 justify-content-end my-2">
                    <button class="btn  btn--primary btn-outline-primary">{{isset($client) ? 'Update' : 'Save'}}</button>
                </div>
            </div> 
        </div>
    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="close client_close_btn" data-dismiss="modal" aria-label="Close">
