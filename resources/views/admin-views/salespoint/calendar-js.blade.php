  <script>
      ...
      function setView(view) {
          let baseUrl = "{{ route('admin.pos.calendar-export') }}";
      ...
      function createDayElement(day, isOtherMonth) {
          var baseUrl =
              "{{ request()->routeIs('admin.inventory.dashboard') || request()->routeIs('admin.inventory.sale.orders') ? route('admin.inventory.report.sale') : route('admin.pos.report') }}";
          ...
          if (!isOtherMonth && sampleData[currentYear] && sampleData[currentYear][currentMonth] && sampleData[currentYear]
              ...
              if (window.currentRoute.startsWith("admin.account.")) {
                  dayData.innerHTML = `
    ...
    `;
              } else if (window.currentRoute.startsWith("admin.task.")) {
                  dayData.innerHTML = `
    ...
    `;
      ...
      function generateYearView() {
          ...
          for (let month = 1; month <= 12; month++) {
              ...
              let totalSales = 0;
              let totalTokens = 0;
              if (window.currentRoute.startsWith("admin.task.")) {
                ...
                `;
              } else if (!window.currentRoute.startsWith("admin.account.")) {
                  if (sampleData[currentYear] && sampleData[currentYear][month]) {
                      Object.values(sampleData[currentYear][month]).forEach(day => {
