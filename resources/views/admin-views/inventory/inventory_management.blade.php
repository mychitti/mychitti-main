...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="page-header d-flex flex-wrap justify-content-between align-items-start w-100">
            ...
            <div class="d-flex gap-2 flex-wrap justify-content-center">
                ...
                <div id="reader"></div>
                <div id="result"></div>
                <a href="{{ route('admin.inventory.item.scan-barcode') }}" class="btn btn-primary btn_sm">Scan Barcode</a>
                                                ...
                                                <tbody id="set-rows">
                                                    @foreach ($inventory_items as $key => $item)
                                                        <tr class="clickable-row"
                                                            data-href="{{ route('admin.inventory.item.detail', [$item->id]) }}">
                                                            ...
                                                            @if (hasPermission('inventory_item', 'show_on_website'))
                                                                <td class="hide_on_phone">
                                                                    <label class="toggle-switch toggle-switch-sm"
                                                                        ...
                                                                        <input type="checkbox"
                                                                            data-url="{{ route('admin.inventory.item.show_on_website', [$item['id'], $item->show_on_store_page ? 0 : 1]) }}"
                                                                    ...
                                                                    <div class="dropdown-menu">
                                                                        @if (hasPermission('inventory_item', 'view'))
                                                                            <a class="dropdown-item text-success"
                                                                                href="{{ route('admin.inventory.item.detail', [$item->id]) }}"
                                                                        ...
                                                                        @if (hasPermission('inventory_item', 'edit'))
                                                                            <a class="dropdown-item text-primary"
                                                                                href="{{ route('admin.inventory.edit-item', [$item->id]) }}"
                                                                        ...
                                                                        @if (hasPermission('inventory_item', 'share'))
                                                                            <a class="dropdown-item text-warning share-btn"
                                                                                ...
                                                                                href="javascript:;"
                                                                                data-url="{{ route('admin.inventory.item.detail', $item->id) }}"
                                                                        ...
                                                                        @if (hasPermission('inventory_item', 'delete'))
                                                                            ...
                                                                            <form
                                                                                action="{{ route('admin.inventory.item.delete', [$item['id']]) }}"
                                                                                method="post"
                                                                                id="item-{{ $item['id'] }}">
                                                                                @csrf @method('get')
                                                                            </form>
                                    ...
                                    <div class="card h-100">
                                        <div class="card-header d-flex flex-wrap p-2">
                                            ...
                                            <div class="d-flex gap-2 flex-wrap">
                                                ...
                                                @if (hasPermission('inventory_item_entry', 'export'))
                                                    <a href="{{ route('admin.inventory.entry.export') }}"
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.inventory.import') }}" method="post"
                    ...
                    <div class="p-3">
                        ...
                        <ol>
                            ...
                            <li><strong>Item Image</strong><br> Upload item images from the <strong><a
                                        class="text-underline" href="{{ route('admin.inventory.item-images') }}">Item
        ...
        <div class="modal fade" id="importEntryExcelModal" tabindex="-1" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.inventory.entry.import') }}" method="post"
                            enctype="multipart/form-data">
                            @csrf
    ...
    @endif
    @if (hasPermission('inventory_item_entry', 'add'))
        @include('admin-views/form_modals/inventory_item_entry')
    @endif

@endsection

@push('script_2')
    @include('admin-views/js/inventory_management')
    <script>
        @if ($tab == 'add')
            $(".add_item_btn").click()
        @endif
        ...
        $("#delete_all").on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.post({
                        url: '{{ route('admin.inventory.item.bulk-delete') }}',
                        data: {
        ...
        $("#download_selected").on('click', function() {
            ...
            var form = $('<form>', {
                action: '{{ route('admin.inventory.item.export-selected') }}',
                method: 'GET'
            });
        ...
        $("#delete_all_entry").on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.post({
                        url: '{{ route('admin.inventory.entry.bulk-delete') }}',
                        data: {
                            item_ids: selectedIds
                        },
                        success: function(data) {
        ...
        $("#download_selected_entry").on('click', function() {
            ...
            var form = $('<form>', {
                action: '{{ route('admin.inventory.entry.export-selected') }}',
                method: 'GET'
            });
