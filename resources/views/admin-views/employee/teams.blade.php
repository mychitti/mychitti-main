                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('admin.staff.team.save') }}" method="post">
                                        @csrf
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <select name="team_leader" id="team_leader">
                                                    ...
                                                    @foreach ($staff as $key => $st)
                                                        <option value="{{ $st->id }}"
                                                            data-image="{{ asset('storage/admin') . '/' . $st->image }}">
                        ...
                        <tbody id="set-rows">
                            @foreach ($teams as $k => $e)
                                <tr>
                                    ...
                                    <td><a
                                            href="{{ hasPermission('staff_manage', 'view') && $e->team_lead ? route('admin.employee.view', [$e->team_lead]) : '#' }}">{{ $e->team_leader?->f_name . ' ' . $e->team_leader?->l_name }}</a>
                                    </td>
                                                        ...
                                                        <div class="avatar avatar-1">
                                                            <div class="avatar-initial">
                                                                <img style="width:100%; height:100%;"
                                                                    src="{{ asset('storage/admin') . '/' . $emp->image }}"
                                    ...
                                    <td> 
                                        @if (auth('admin')->id() != $e['id'])
                                            <div class="btn--container justify-content-center">
                                                @if (hasPermission('staff_team', 'edit'))
                                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                                        href="{{ route('admin.staff.team.edit', [$e['id']]) }}"
                                                        title="Edit team"><i class="tio-edit"></i>
                                                    </a>
                                                @endif
                                                @if (hasPermission('staff_team', 'view'))
                                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                                        href="{{ route('admin.staff.team.view', [$e['id']]) }}"
                                            ...
                                            @if (hasPermission('staff_team', 'delete'))
                                                <form action="{{ route('admin.staff.team.delete', [$e['id']]) }}"
...
@push('script_2')
    ...
    <script>
        function formatOption(option) {
            ...
            return $(`
      <span>
        <img src="${image}" onerror="this.src='{{ asset('public/assets/admin/img/160x160/img1.jpg') }}'" style="width: 24px; height: 24px; border-radius: 5px; margin-right: 8px;" />
        ${option.text}
      </span>
    `); 
        } 
