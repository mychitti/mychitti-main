                    ...
                    <!-- Search -->
                        ...
                        <div class="input-group input--group">
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{translate('Name or Phone')}}" aria-label="{{translate('messages.search')}}" value="{{isset($search_by) ? $search_by : ''}}" required>
            ...
            <div class="table-responsive datatable-custom">
                <table id="columnSearchDatatable"
                    ...
                    <tr>
                        ...
                        <th class="border-0">Denied By</th>
                        <th class="border-0">Deny Reason</th>
                    ...
                    @php if($store->status_updated_by){
                       $empInfo =  _getOneWhere('admins', ['id' => $store->status_updated_by]);
                    } @endphp  
                        <tr>
                            ...
                            <td>{{explode(' ',$store->created_at)[0]}}</td>
                            <td>{{explode(' ',$store->created_at)[1]}}</td>
                            <td>{{$store->vendor?->createdBy?->f_name . ' ' . $store->vendor?->createdBy?->l_name . ' #' . $store->vendor?->created_by}}</td>
                            <td>{{$store->status_updated_by ? $empInfo->f_name . ' ' . $empInfo->l_name : '-'}}</td>
                            <td>{{$store->reason}}</td>
                            <td>
