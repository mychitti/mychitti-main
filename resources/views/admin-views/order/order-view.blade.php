                    ...
                    <div class="card-header border-0 align-items-start flex-wrap">
                        <div class="order-invoice-left d-flex d-sm-block justify-content-between">
                            ...
                            @if($order['invoice_id'])
                                ...
                                @endif
                                <h6 class="page-header-title d-flex align-items-center __gap-5px">
                        ...
                        <div class="order-invoice-right mt-3 mt-sm-0">
                            <div class="btn--container ml-auto align-items-center justify-content-end">
                                ...
                                @if (0 &&   !$parcel_order &&  !$editing && in_array($order->order_status, ['pending', 'confirmed', 'processing', 'accepted']) &&
                                        isset($order->store) &&
                                ...
                                <a class="btn btn--primary print--btn font-regular d-none d-sm-block"
                                    href={{ route('admin.order.generate-order-invoice', [$order['id']]) }}>
                                    <i class="tio-print mr-sm-1"></i> <span>{{ translate('messages.download_invoice') }}</span>
                                </a>
                                <a class="btn btn--primary print--btn font-regular d-none d-sm-block"
                                    href={{ route('admin.order.generate-invoice', [$order['id']]) }}>
                                    <i class="tio-print mr-sm-1"></i> <span>{{ translate('messages.print_cash_reciept') }}</span>
                                </a>
                            </div>
                                        ...
                                        @foreach ($details as $key => $detail)
                                            @if (isset($detail->item_id) && $detail->status)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <div>
                                                            @php($amount = isset($detail['mrpprice']) ? $detail['mrpprice'] * $detail['quantity'] : $detail['price'] * $detail['quantity'])
                                    ...
                                    <span class="delivery--information-single mt-3">
                                        ...
                                        <span class="name">{{ translate('messages.contact') }}</span>
                                               ...
                                               <a href="javascript:;" style="cursor:default;"
                                                                    class="textToCopy deco-none info d-flex">{{$receiver_details['contact_person_number'] }}</a>
                                                                <button
                            ...
                            @if (isset($address))
                                <div class="delivery--information-single mt-3">
                                    ...
                                    <span class="name">{{ translate('messages.contact') }}</span>
                                         ...
                                         <a href="javascript:;" style="cursor:default;"
                                            class="textToCopy">{{ data_get($address,'contact_person_number', translate('messages.N/A'))  }}</a>
                                        <button
                                            class="copy-btn bg-transparent outline-none border-0">
                                            <i class="tio-copy"></i>
                                        </button>
                    ...
                    <div class="card mt-2">
                        ...
                        <div class="card-body">
                            ...
                            <a class="media align-items-center deco-none resturant--information-single"
                                ...
                                <div class="media-body">
                                    <span class="fz--14px text--title font-semibold text-hover-primary d-block">
                                        {{ $order->store['name'] }} {{$order->store['vendor_type'] == 'composition' ? '(Composition Vendor)' : ''}}
                                    </span>
...
@push('script_2')
    ...
    <script>
        ...
        function initMap() {
            ...
            @if ($order->store)
                $.get({
                    url: '{{ url('/') }}/zone/get-coordinates/{{ $order->store->zone_id }}',
                    dataType: 'json',
                    success: function(data) {
