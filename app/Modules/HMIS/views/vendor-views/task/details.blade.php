                ...
                <div class="d-flex flex-wrap">
                    ...
                    <div class="ml-2">
                        @if (
                            ...
                            <a href="{{ $task->parent_id ? route('vendor.task.subtask.edit', [$task->id]) : route('vendor.task.edit', [$task->id]) }}"
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="d-flex gap-2">
                                                                    @if (!$task_closed && hasPermission($task_type . '_update', 'edit'))
                                                                        <a href="{{ $task->parent_id ? route('vendor.task.subtask-update.edit', [$comment['id']]) : route('vendor.task.comment.edit', [$comment['id']]) }}"
                                                                            class="btn btn-sm btn-outline-primary">
                                                                            <i class="tio-edit"></i>
                                                                        </a>
                                                                    @endif
                                                                    @if (!$task_closed && hasPermission($task_type . '_update', 'delete'))
                                                                        <a href="{{ $task->parent_id ? route('vendor.task.subtask-update.delete', [$comment['id']]) : route('vendor.task.comment.delete', [$comment['id']]) }}"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="add-task-section">
                                <form method="post" id="add_task_form" action="{{ route('vendor.task.subtask.add') }}">
                                    @csrf
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="customer_add_form" enctype="multipart/form-data" class="w-100"
                                action="{{ $task->parent_id ? route('vendor.task.subtask-update.add') : route('vendor.task.comment.add') }}"
                                method="post">
                                @csrf
                        ...
                        <div class="modal-body">
                            <div class="row align-items-end">
                                <form class="customer_add_form status_update_form" enctype="multipart/form-data"
                                    class="w-100"
                                    action="{{ $task->parent_id ? route('vendor.task.subtask.status.update') : route('vendor.task.status.update') }}"
                                    method="post">
                                    @csrf
...
@push('script_2')
    ...
    <tr class="item_row row_${dataId}" data-id="${dataId}">
        ...
        function handleTaskDrop(e, targetTask) {
            ...
            $.get({
                url: "{{ route('vendor.task.subtask.update-level') }}",
                data: {
                    parent_id: parent_id,
                    child_id: child_id
                },
        ...
        function createTaskElement(task, level) {
            ...
            const taskDetailUrl = "{{ route('vendor.task.subtask.detail', [':id']) }}";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('{{ route('vendor.task.subtask.getTasks', $task->id) }}')
                .then(response => response.json())
                .then(data => {
                    tasks = data;
                    renderTasks();
    ...
    <script>
        $(document).ready(function() {
            ...
            function afUpdateProgress(value, save = false) {
                ...
                if (save) {
                    $.ajax({
                        url: '{{ route('vendor.task.save-progress') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            task_id: $taskId,
