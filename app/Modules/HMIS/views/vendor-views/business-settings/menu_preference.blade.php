            ...
            <div class="col-md-8 card shadow-sm border-0 rounded-3 overflow-hidden">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('vendor.menu_preference_save') }}" method="POST">
                                            ...
                                            @foreach ($menus as $key => $value)
                                                    <div class="col-md-3 col-sm-6 mb-2">
                                                        ...
                                                        menu_card menu_card_{{ $value->slug }} {{ in_array($value->slug, $selectedMenus) ? 'active' : '' }}"
                                                            ...
                                                            <input @if (
                                                                ($value->business_type != 'all' && strtolower($value->business_type) != strtolower($store_business_type)) ||
                        ...
                        {{-- Tell the save which slugs were rendered so it only clears those --}}
                        @foreach($full_menu as $item)
                            <input type="hidden" name="visible_slugs[]" value="{{ $item->slug }}">
                        @endforeach
