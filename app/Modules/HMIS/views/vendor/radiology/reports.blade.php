        ...
        <div class="card">
            ...
            @forelse ($studies as $s)
                <div class="tbl-row" style="{{ $cols }}">
                    ...
                    <div><div style="font-weight:700">{{ $s->patient->name ?? '—' }}</div><div style="font-size:10px;color:var(--light)">{{ $s->patient->patient_uid ?? '' }}</div></div>
                    <div><span class="pill {{ $modPill($s->modality) }}">{{ $s->modality }}</span></div>
                    <div style="font-size:11px">{{ $s->study_name }}</div>
                    <div style="font-size:11px;color:var(--muted)">{{ $s->reported_at?->format('d M Y') ?? $s->updated_at?->format('d M Y') }}</div>
                    <div>@if($s->status==='sent')<span class="pill pill-green">Sent</span>@else<span class="pill pill-teal">Verified</span>@endif</div>
