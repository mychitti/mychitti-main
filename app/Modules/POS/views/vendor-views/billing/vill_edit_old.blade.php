...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="row g-2">
            <form class="w-100 row" action="{{ route('vendor.invoice.update-invoice') }}" method="post">
                @csrf
                    ...
                    <div class="card h-100">
                        <div class="card-body row  align-items-start">
                            <div class="form-check  col-md-3  p-1">
                                <label class="form-check-label d-flex " for="flexRadioDefault2">Bill
                                    to
                                    {{-- <i onclick="hideCustomerInput()" {{ !$invoice->bill_to ? "style=display:none;" : '' }}
                                        class="delete_icon tio-delete-outlined border rounded mx-1 p-1 border-danger text-danger cursor-pointer"></i>
                                    <i onclick="showCustomerInput()" {{ !$invoice->bill_to ? '' : 'style=display:none;' }}
                                        class="add_icon tio-add border rounded mx-1 p-1 border-primary text--primary cursor-pointer"></i> --}}
                                </label>
                                <div id="customer_id_elem" >
                                    <select name="bill_to" id="customer_id" class="form-control js-select2-custom">
                                        <option value=""></option>
                                        <option value="add_new">Add New</option>
                                        @foreach ($customers as $cust)
                                            <option {{ $invoice->bill_to == $cust->id ? 'selected' : '' }}
                                                value="{{ $cust->id }}">
