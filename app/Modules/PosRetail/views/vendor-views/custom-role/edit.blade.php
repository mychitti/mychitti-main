            ...
            <div class="card-body">
                <form class="role_form" action="{{ route('vendor.custom-role.update', [$role['id']]) }}" method="post">
                    ...
                    <div class="check--item-wrapper mx-0">
                        ...
                        @endif
                        @if(vendorPlanHasModule('pos_retail'))
                        <div class="check-item">
                            <div class="form-group form-check form--check">
                                <input type="checkbox" name="modules[]" value="pos_retail"
                                    class="form-check-input granular_permission_check" id="pos_retail"
                                    {{ in_array('pos_retail', (array) json_decode($role['modules'])) ? 'checked' : '' }}>
                                <label class="form-check-label " for="pos_retail">Retail POS</label>
                            </div>
                        </div>
                        @endif
                    ...
                    @php
                        // Ensure the Retail POS billing permission exists + sits in its own group.
                        if (strtolower(\App\CentralLogics\Helpers::get_store_data()->business_type ?? '') === 'pos_retail'
                            && class_exists(\App\Modules\PosRetail\Controllers\Vendor\RetailPosController::class)) {
                            \App\Modules\PosRetail\Controllers\Vendor\RetailPosController::seedPermissions();
                        }
                    ...
                    @foreach ($features as $moduleName => $moduleFeatures)
                        ...
                        <h5 class="mt-4 master_module_heading" data-master-module="{{ $moduleName }}"
                            ...
                            {{ $moduleName === 'pos_retail' ? 'Retail POS' : ucfirst($moduleName) }}
                        </h5>
