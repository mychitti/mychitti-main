    ...
    <div class="content container-fluid">
        ...
        <div class=" d-flex gap-3 row">
            @if (hasPermission('apporval_form_journal_entry', 'add'))
                <div class="card shadow rounded  col-md-7">
                        ...
                        <div
                            ...
                            <div class="logo-container">
                                <div class="company-logo">
                                    <img src="{{ asset('storage/app/public/store/' . \App\CentralLogics\Helpers::get_store_data()->logo) }}"
                                        alt="">
                                </div>
                            </div>
                        </div>
                    ...
                    <form
                        action="{{ $edit ? route('vendor.account.request-form.journal-entry.update') : route('vendor.account.request-form.journal-entry.store') }}"
                            ...
                            <div class="row px-2">
                                ...
                                <div class="col-md-4 ">
                                    <div class="d-flex my-2 gap-2 align-items-center">
                                        ...
                                        <input type="date"
                                            value="{{ $edit && $edit->date ? $edit->date : date('Y-m-d') }}" name="date"
                        ...
                        <div class="vendor-header py-1 mb-0">
                            <div class="row px-2">
                                <div class="col-md-5 ">
                                    <div class="d-flex my-2 gap-2 align-items-center">
                                        <label class="form-label label_lg" style="white-space: nowrap">Requested To</label>
                                        <select name="request_to" id="" data-placeholder="Select Requested To"
                                            class="js-select2-custom">
                                            <option value=""></option>
                                            @foreach ($employees as $key => $value)
                                                <option {{ $edit && $edit->request_to == $value->id ? 'selected' : '' }}
                                                    value="{{ $value->id }}">
                                                    {{ $value->f_name . ' ' . $value->l_name }}
                                                </option>
                                            @endforeach
                        ...
                        <div class="vendor-header py-1 mb-0">
                            <div class="row px-2">
                                <div class="col-md-6 ">
                                    <div class="d-flex my-2 gap-2 align-items-center">
                                        <label class="form-label label_lg " style="white-space: nowrap">Ledger Type</label>
                                        <select name="ledger_type" id="" data-placeholder="Select Ledger Type"
                                            class="js-select2-custom">
                                            <option value=""></option>
                                            @foreach ($ledger_types as $key => $value)
                                                <option {{ $edit && $edit->ledger_type == $value->id ? 'selected' : '' }}
                                                    value="{{ $value->id }}">{{ $value->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="d-flex my-2 gap-2 align-items-center">
                                        <label class="form-label label_lg" style="white-space: nowrap">Description</label>
                                ...
                                <div class="col-md-4 my-2">
                                    <div class="d-flex h-100 align-items-end gap-2">
                                        ...
                                        <input type="number" step="0.001" value="{{ $edit?->amount }}"
                                            placeholder="Ex: 1200" name="amount" class="form-control" id="amount_inp">
                                    </div>
                                </div>
                            </div>
                        </div>
                                                    ...
                                                    @if (hasPermission('apporval_form_journal_entry', 'edit'))
                                                        @if ($req->resubmit <= $permitted_resubmit)
                                                            <a style="padding: 0 10px ; width: fit-content;"
                                                                ...
                                                                href="{{ route('vendor.account.request-form.journal-entry.index', [$req->id]) }}"
