...
@section('content')
 ...
 <a class="dropdown-item text-success form-alert"
    ...
    @endif
    @if (hasPermission('inventory_item_entry', 'add'))
        {{-- Manage Storage Unit modal — opened by the "Stock Management" button. --}}
        @include('vendor-views/form_modals/storage_unit_manage')

        {{-- BACKUP: original full stock-entry modal (#itemEntryModal). Kept for re-enabling —
             repoint the Stock Management button's data-target back to #itemEntryModal to restore. --}}
