    ...
    <div class="content container-fluid">
        ...
        <div class="card-header d-flex justify-content-end gap-2 flex-wrap py-2">
            ...
            @if (hasPermission('project_task', 'add'))
                <a href="{{ route('vendor.project.task.add') }}" class="btn btn_sm btn-primary">+ Add New Task</a>
            @endif
        </div>
        <div class="page-header d-flex justify-content-between flex-wrap">
            ...
            <div class="card-header d-flex py-2 align-items-center">
                ...
                @if (!$empId) <!-- if not showing employee specific -->
                    ...
                    @endif
                    @if (hasPermission('project_task', 'export'))
                        <a class="btn btn-outline-primary mr-1" href="{{ route('vendor.project.task.export') }}">
                            Export
                        </a>
                    @endif
                    @if (hasPermission('project_task', 'list'))
                ...
                <div class="d-flex gap-4 py-4 px-2 flex-wrap">
                    ...
                    @foreach ($tasks as $key => $task)
                        ...
                        <div class="repair-card-container {{ $status . '-color' }} "
                            @if (hasPermission('project_task', 'view')) onclick="handleClick('{{ route('vendor.project.task.detail', [$task->id]) }}', event)" @endif>
                                    ...
                                    <div class="dropdown-menu">
                                        @if (hasPermission('project_task', 'view'))
                                            <a class="dropdown-item text-warning"
                                                href="{{ route('vendor.project.task.detail', [$task->id]) }}"
                                                title="{{ translate('messages.view') }}"><i
                                                    class="tio-visible-outlined"></i>
                                                View
                                            </a>
                                        @endif
                                        @if (hasPermission('project_task', 'edit'))
                                            ...
                                            @endif
                                            @if ($task->employee_id == \App\CentralLogics\Helpers::get_loggedin_user()->id || hasPermission('project_task', 'edit_others'))
                                                <a class="dropdown-item  text--primary"
                                                    href="{{ $task->parent_id ? route('vendor.project.task.subtask.edit', [$task->id]) : route('vendor.project.task.edit', [$task->id]) }}"
                                        ...
                                        @if (hasPermission('project_task', 'delete'))
                                            ...
                                            <form
                                                action="{{ $task->parent_id ? route('vendor.project.task.subtask.delete', [$task['id']]) : route('vendor.project.task.delete', [$task['id']]) }}"
