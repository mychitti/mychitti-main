        ...
        <div class="page-header d-flex justify-content-between flex-wrap">
            <div class="card-header d-flex py-2 align-items-center">
                ...
                @if (!$empId) <!-- if not showing employee specific -->
                    ...
                    <a class="btn btn-outline-primary mr-1" href="{{ route('vendor.task.export') }}">
                        Export
                    </a>
            ...
            <div class="d-flex gap-4 py-4 px-2 flex-wrap">
                ...
                @foreach ($tasks as $key => $task)
                    ...
                    <div class="repair-card-container {{ $status . '-color' }} "
                        onclick="handleClick('{{ $task->parent_id ?  ($task->employee_id == null ? '#' : route('vendor.task.subtask.detail', [$task->id]) ) : ($task->employee_id == null ? '#' : route('vendor.task.detail', [$task->id]) )}}', event)">
                        ...
                        <div class="repair-card-content">
                                ...
                                <div style="margin-bottom: 5px;" class="menu_section">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item text-warning"
                                            href="{{ $task->parent_id ? route('vendor.task.subtask.detail', [$task->id]) : route('vendor.task.detail', [$task->id]) }}"
                            ...
                            @elseif($task->employee_id == null)
                                <div class="d-flex gap-2 w-100 justify-content-center mt-3">
                                    ...
                                    <form action="{{ route('vendor.task.accept', [$task['id']]) }}" method="get"
                                        id="category-{{ $task['id'] }}">
                                        @csrf @method('get')
                                    </form>
                                    <button style="width:fit-content; padding: 0 10px !important;"
                                    ...
                                    <form action="{{ route('vendor.task.reject', [$task['id']]) }}" method="get"
