    ...
    <div class="page-header d-flex align-items-center justify-content-between gap-2 flex-wrap">
        ...
        <div class="d-flex gap-2 flex-wrap align-items-center">
            <a href="{{ route('vendor.laundry.register.export', array_filter(['date_range' => $preset, 'custom_date_range' => request('custom_date_range'), 'hotel_id' => $hotelId])) }}"
               class="btn btn-outline-success btn_sm"><i class="tio-download"></i> Export Excel</a>
            <button onclick="window.print()" class="btn btn-outline-secondary btn_sm"><i class="tio-print"></i> Print</button>
            @if($hotelId)
            <a href="{{ route('vendor.laundry.register.billing', array_filter(['hotel_id' => $hotelId, 'date_range' => $preset, 'custom_date_range' => request('custom_date_range')])) }}"
               class="btn btn--primary btn_sm"><i class="tio-receipt"></i> Generate Bill</a>
            @else
            <button class="btn btn--primary btn_sm" disabled title="Select a hotel to generate bill">
