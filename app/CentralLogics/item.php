...
class ProductLogic
{
    ...
    public static function get_latest_products($zone_id, $limit, $offset, $store_id, $category_id, $type, $min = false, $max = false, $product_id = null, $module_id = null)
    {
            ...
            $paginator = Item::join('stores', function ($join) {
                ...
                ->whereHas('category', function ($query) use ($category_id) {
                    $query->when(is_numeric($category_id), function ($query) use ($category_id) {
                        $query->whereId($category_id)->orWhere('parent_id', $category_id);
                    })
                        ->when(!is_numeric($category_id), function ($query) use ($category_id) {
                            $query->where('slug', $category_id);
                        });
                })
                ->select('items.*', 'stores.zone_id')
                ->distinct('items.id')
