...
class CategoryController extends Controller
{
    ...
    public function get_all_products($id, Request $request)
    {
        if ($request->header('moduleId') == 6) {
            ...
            return CategoryLogic::all_services($id, $zone_id);
        } else {
            if (!$request->hasHeader('zoneId')) {
                $errors = [];
                array_push($errors, ['code' => 'zoneId', 'message' => translate('messages.zone_id_required')]);
            ...
            // prx(CategoryLogic::all_products($id, $zone_id), true, false, app()->getLocale());

            try {
                return CategoryLogic::all_products($id, $zone_id);
            } catch (\Exception $e) {
                return response()->json([], 200);
            }
        }
    ...
    public function get_all_services($id, Request $request)
    {
        // if (!$request->hasHeader('zoneId')) {
        //     $errors = [];
        //     array_push($errors, ['code' => 'zoneId', 'message' => translate('messages.zone_id_required')]);
        ...
        // return response()->json(CategoryLogic::all_serivces($id, $zone_id));
