 <?php    public function place_order(Request $request)
    {
        ...
        if ($request->order_type !== 'parcel') {
            ...
            if ($store->free_delivery ||  $store->delivery_charges_on <  $order->order_amount) {
                $order->delivery_charge = 0;
        ...
        if ((!$store->free_delivery && $store->delivery_charges_on > $order->order_amount) && (isset($module_wise_delivery_charge) && $request->payment_method == 'cash_on_delivery' && $module_wise_delivery_charge->pivot->maximum_cod_order_amount && $order->order_amount > $module_wise_delivery_charge->pivot->maximum_cod_order_amount)) {
            return response()->json([
                'errors' => [
