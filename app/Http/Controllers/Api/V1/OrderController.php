...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
                    ...
                    if ($product) {
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            if (count($product_variations)) {
                                ...
                                $taxable_price = $variation_data['price'];
                                $variations =  $variation_data['variations'];
                                $tax_amount = Helpers::calc_tax($variation_data['price'],  $variation_data['tax']);
                                $mychitti_fee = $variation_data['price'] - $variation_data['askingprice'];
                            } else {
                                $price = $product['price'];
                                $taxable_price = $product['price'];
                                $mychitti_fee = $product->mychitty_fee;
                                $tax_amount  = Helpers::tax_calculate_product($product->id);
                            }
                            $product->tax = $store->tax;
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                            $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id', $c['add_on_ids'])->get(), $c['add_on_qtys']);
                            ...
                            $or_d = [
                                ...
                                'quantity' => $c['quantity'],
                                'price' => $price,
                                'tax_amount' => $tax_amount ?? Helpers::tax_calculate($product, $taxable_price),
                            ...
                            if (count(json_decode($product['variations'], true)) > 0 && count($c['variation']) > 0) {
                                ...
                                $stock = $variant_data['stock'];
                                $is_vr = true;
                                $discount23 = $variant_data['discount'];
                                $discount_type23 = 'percent';
                                $mychitti_fee = $variant_data['price'] - $variant_data['askingprice'];
                            } else {
                                $price = $product['price'];
                                $stock = $product->stock;
                                $is_vr = false;
                                $discount23 = $product['discount'];
                                $discount_type23 = $product['discount_type'];
                                $mychitti_fee = $product->mychitty_fee;
                            }
                            ...
                            if (isset($c['variation']) && $c['variation']) {
                                $tax = isset($c['variation'][0]['tax']) ? $c['variation'][0]['tax'] : 0;
                                $tax_type = 'percent';
                                $mrpprice = $c['variation'][0]['mrpprice'] ?? $c['variation'][0]['price'];
                                $price = $c['variation'][0]['price'];
                                $tax_amount  = Helpers::tax_calculate_new($tax, $tax_type, $price);
                            ...
                            $total_addon_price += $or_d['total_add_on_price'];
                            if ($is_vr) {
                                $product_price += ($price * $or_d['quantity'] + ($tax_amount * $or_d['quantity']));
                            } else {
                                $product_price += ($price * $or_d['quantity'] + $tax_amount * $or_d['quantity']);
                            }
                            $store_discount_amount += $or_d['discount_type'] != 'flash_sale' ? $or_d['discount_on_item'] * $or_d['quantity'] : 0;
        ...
        try {
            ...
            try {
                ...
                //     if (!in_array($order->payment_method, ['digital_payment', 'partial_payment', 'offline_payment'])  || $payments) {
                //         Helpers::send_order_place_notification($order) ;
                //     }
    ...
    public function download_bill(Request $request)
    {
        ...
        $existing = OrderInvoice::where('order_id', $id)->first();
        if ($existing) {
            $inv = OrderInvoice::where('order_id', $id)->first();

            $pdfPath = storage_path('app/public/invoice/' . $inv->pdf);
        ...
        $html =  FacadesView::make('front-views.partials._invoice', compact('order'));
        $mpdf->WriteHTML($html);
