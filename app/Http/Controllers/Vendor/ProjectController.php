...
class ProjectController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        return view('vendor-views.project.dashboard', compact('projectNames', 'progress', 'stats'));
    }
    public function settings(Request $request)
    {
        $storeConfig = StoreConfig::where('store_id', Helpers::get_store_id())->first();
        return view('vendor-views.project.setting', compact('storeConfig'));
    }
    public function add(Request $request)
    {
        $storeId = Helpers::get_store_id();
        $employees = VendorEmployee::where('store_id', $storeId)->with(['role'])->get();
        ...
        return view('vendor-views.project.add_project', compact('employees', 'departments', 'categories'));
    }
    public function edit(Request $request, $id)
    {
        ...
        $departments = Department::where('vendor_id', $storeId)->get();
        $categories = AccountOption::where('store_id', $storeId)->where('type', 'project_category')->get();
        return view('vendor-views.project.edit_project', compact('employees', 'departments', 'project', 'categories'));
    }
    public function task_list(Request $request, $project_id = null)
    {
        ...
        return view('vendor-views.project.task_list', compact('tasks', 'project'));
    }
    public function index(Request $request, $empId = null)
    {
        ...
        if ($empId) {
            $projects = Project::with('client', 'tasks', 'teamMembers')->where('team_leader', $empId)->where('vendor_id', Helpers::get_store_id())->get();
        } else {
            $projects = Project::with('client', 'tasks', 'teamMembers')->where('vendor_id', Helpers::get_store_id())
        ...
        $employees = VendorEmployee::where('store_id', Helpers::get_store_id())->with(['role'])->get();

        return view('vendor-views.project.index', compact('empId', 'projects', 'employees', 'preset'));
    }
    ...
    public function udpate_team(Request $request)
    {
        ...
        $oldMembers = ProjectTeamMember::where('project_id', $project_id)->pluck('employee_id')->toArray();
        ...
        return redirect()->route('vendor.project.details', [$project_id, 'team']);
    }
    ...
    public function tasks_list(Request $request)
    {
        ...
        // prx($tasks);
        return view('vendor-views.project.task_list', compact('staff', 'sales', 'preset', 'data',  'tasks', 'statuses', 'from', 'to', 'status'));
    }
    public function details(Request $request)
    {
        $store_id = Helpers::get_store_id();

        $project =  Project::with('tasks', 'client', 'projectManager', 'attachments', 'teamMembers.employee', 'internalNotes')->where('id', $request->id)->first();
        $titles = ProjectTask::where('project_id', $project['id'])->select('title')->distinct()->get();
        $team_leader = VendorEmployee::find($project['team_leader']);
        $assigned_to = VendorEmployee::find($project['team_members']);
        $tasks = ProjectTask::where('project_id', $project['id'])->get();
        $comments = ProjectComment::where('project_id', $project['id'])->get();
        $employees = VendorEmployee::where('store_id', $store_id)->with(['role'])->get();

        return view('vendor-views.project.details', compact('project', 'titles', 'employees', 'team_leader', 'assigned_to', 'tasks', 'comments'));
    }
    ...
    public function save_info_old(Request $request)
    {
        ...
        if ($id == '') { // for new project
            ...
            $project = new Project;
            $project->vendor_id = Helpers::get_store_id();
        } else {
    ...
    public function save_info(Request $request)
    {
        ...
        $oldMembers = ProjectTeamMember::where('project_id', $project->id)->pluck('employee_id')->toArray();
        $newMembers = $request->team_members ?? [];
    ...
    public function store_milestone(Request $request)
    {
        ...
        return redirect()->route('vendor.project.details', [$project_id, 'milestones']);
    }
    ...
    public function store_note(Request $request)
    {
        ...
        return redirect()->route('vendor.project.details', [$project_id, 'notes']);
    }
}
