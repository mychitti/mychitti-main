...
class WalletController extends Controller
{
    ...
    public function wallet_recharge_w_coupon(Request $request)
    {
    ...
    {
        // prx(Helpers::get_vendor_id());
        $data =  data_get($this->getWithdrawMethods(), 'data', []);
        $withdrawal_methods =  data_get($this->getWithdrawMethods(), 'withdrawal_methods', []);

        $key = isset($request['search']) ? explode(' ', $request['search']) : [];
        $account_transaction = AccountTransaction::when(isset($key), function ($query) use ($key) {
