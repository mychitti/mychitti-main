...
class LaundryController extends Controller
{
    ...
    public function dashboard(\Illuminate\Http\Request $request)
    {
        ...
        // Recent records (filtered by selected date range)
        ...
        $sales = Helpers::laundry_calendar();

        return view('vendor-views.laundry.dashboard', compact(
    ...
    public function items(Request $request)
    {
        ...
        $query = LaundryItem::where('store_id', $storeId)->orderBy('category')->orderBy('name');
        if ($category) {
            $query->where('category', $category);
        }

        $items = $query->get();
        return view('vendor-views.laundry.items', compact('items', 'category'));
    }
    ...
    public function item_store(Request $request)
    {
        $request->validate([
            ...
            'category' => 'required|in:normal,premium,dry_clean',
            'price'    => 'required|numeric|min:0',
            'unit'     => 'required|in:piece,kg,pair',
        ]);
    ...
    public function item_update(Request $request, $id)
    {
        ...
        $item = LaundryItem::where('id', $id)->where('store_id', Helpers::get_store_id())->firstOrFail();
        $item->update([
            'name'     => $request->name,
            'category' => $request->category,
            'price'    => $request->price,
    ...
    public function item_toggle(Request $request, $id)
    {
        $item = LaundryItem::where('id', $id)->where('store_id', Helpers::get_store_id())->firstOrFail();
    ...
    public function orders(Request $request)
    {
        ...
        return view('vendor-views.laundry.orders', compact('orders', 'status', 'search', 'from', 'to', 'preset'));
    }

    public function order_create()
    {
        $storeId = Helpers::get_store_id();
        $items   = LaundryItem::where('store_id', $storeId)->where('is_active', 1)->orderBy('category')->orderBy('name')->get();
        return view('vendor-views.laundry.order_create', compact('items'));
    }
        ...
        try {
            ...
            foreach ($request->items as $row) {
                LaundryOrderItem::create([
                    ...
                    'laundry_item_id'  => $row['laundry_item_id'] ?? null,
    ...
    public function order_show($id)
    {
        ...
        $order   = LaundryOrder::with('items', 'customer', 'employee')
            ->where('store_id', $storeId)
            ->findOrFail($id);
        return view('vendor-views.laundry.order_show', compact('order'));
    }
    ...
    public function challans(Request $request)
    {
        ...
        return view('vendor-views.laundry.challans', compact('challans', 'hotels', 'status', 'hotelId', 'from', 'to', 'preset'));
    }
    ...
    public function challan_create()
    {
        $storeId = Helpers::get_store_id();
        $items   = LaundryItem::where('store_id', $storeId)->where('is_active', 1)->orderBy('category')->orderBy('name')->get();
        return view('vendor-views.laundry.challan_create', compact('items'));
    }
    ...
    public function challan_store(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->items as $row) {
                $prevBalance = $this->getPreviousBalance($storeId, $request->store_customer_id, $row['laundry_item_id'] ?? null, $row['item_name']);
    ...
    public function challan_show($id)
    {
        ...
        $challan  = LaundryChallan::with('items', 'hotel')
            ->where('store_id', $storeId)
            ->findOrFail($id);
        return view('vendor-views.laundry.challan_show', compact('challan'));
    }
    ...
    public function challan_receive($id)
    {
        ...
        $challan = LaundryChallan::with('items', 'hotel')
            ...
            ->findOrFail($id);
        return view('vendor-views.laundry.challan_receive', compact('challan'));
    }
    ...
    public function challan_print($id)
    {
        ...
        $challan = LaundryChallan::with('items', 'hotel', 'store')
            ->where('store_id', $storeId)
            ->findOrFail($id);
        return view('vendor-views.laundry.challan_print', compact('challan'));
    }
    ...
    public function monthly_register(Request $request)
    {
        ...
        // Build grid: item_name => [date => net_qty (sent - damaged)]
        $grid = [];
        foreach ($rows as $row) {
        ...
        return view('vendor-views.laundry.monthly_register', compact('grid', 'dates', 'from', 'to', 'preset', 'hotels', 'hotelId'));
    }
    ...
    public function monthly_register_export(Request $request)
    {
        ...
        // Determine column label format (day-only when single month, else d-M)
    ...
    // ================================================================
    //  MONTHLY BILLING — consolidated bill from monthly register
    // ================================================================
...
// prx($challanIds);
    ...
    // ================================================================
    //  ORDER RECEIPT — token receipt print (walk-in drop-off)
    // ================================================================
    ...
    public function order_receipt($id)
    {
        ...
        $order   = LaundryOrder::with('items', 'customer')
            ...
            ->findOrFail($id);
        $store   = Store::find($storeId);
        return view('vendor-views.laundry.order_receipt', compact('order', 'store'));
    }

    // ================================================================
    //  ORDER INVOICE — generate ManualInvoice for walk-in on delivery
    // ================================================================
    ...
    // ================================================================
    //  CHALLAN INVOICE — generate ManualInvoice for hotel challan
    // ================================================================
    ...
    private function getPreviousBalance($storeId, $hotelId, $laundryItemId, $itemName): int
    {
        // Find the last challan for this hotel and item
        ...
        $lastItem = LaundryChallanItem::where('laundry_challan_id', $lastChallanId)
            ->when($laundryItemId, fn($q) => $q->where('laundry_item_id', $laundryItemId))
            ->when(!$laundryItemId, fn($q) => $q->where('item_name', $itemName))
            ->first();
