...
class AccountReportController extends Controller
{
    ...
    public function tax(Request $request, $export = null)
    {
        ...
        return view('vendor-views.account.report.tax', compact('preset','serviceInvoices', 'invoices', 'saleInvoices', 'purchaseInvoices'));
    }
    ...
    public function  audit_logs(Request $request) {
        ...
        $audit_logs = AuditLog::with('employee')
        ->when($search, function ($query) use ($search) {
            $query->where('action', 'like', '%' . $search . '%');
        })
        ->whereBetween('created_at', [$formatted_from, $formatted_to])
        ->where('store_id', $storeId)->paginate(50);
        return view('vendor-views.account.report.audit-logs', compact('preset','audit_logs' ));
    }
}
