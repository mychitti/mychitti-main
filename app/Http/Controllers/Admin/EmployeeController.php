...
class EmployeeController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        if(!$v_id){
          ...
          $em = VendorEmployee::where('store_id',$v_id)->with(['role'])->latest()->paginate(config('default_pagination'));
    ...
    public function edit($id)
    {
        ...
        $rls = EmployeeRole::where('store_id',Helpers::get_store_id())->get();
        $department = Department::where('vendor_id',Helpers::get_store_id())->where('status', '1')->get();
