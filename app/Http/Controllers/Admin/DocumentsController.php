...
class DocumentsController extends Controller
{
    ...
    public function service_report(Request $request, $action = 'view', $task_id = null, $lead_id = null, $reponse_type = 'redirect')
    {
        ...
        if ($task_id) {
            ...
            if ($task) {
                ...
                if ($reportExists) {
                    if ($reponse_type == 'json') {
                        return response()->json(['status' => false, 'message' => 'This task already has a service report']);
                    }
            ...
            $data['client'] = StoreCustomer::where('store_id', 0)
                ->where('id', $customer_id)
                ->first();
        }
        if ($request->employee_id) {
            $data['employee'] = Admin::where('id', $request->employee_id)
                ->first();
        } else {
            $vendor_id = 0;
        ...
        if ($action == 'save') {
            ...
            $serviceReport->created_by = auth('admin')->user()->role_id == 1 ? 0 : auth('admin')->id();
            $serviceReport->save();
        }
    ...
    public function add_gatepass(Request $request, $gp_type)
    {
        ...
        // prx($routes);
        return view('admin-views.documents.gatepass.index', compact('staff', 'routes', 'gp_type', 'vehicles'));
    }
    ...
    public function service_report_store(Request $request, $action = 'view', $task_id = null, $lead_id = null)
    {
        ...
        $save =  $this->service_report($request, 'save',  $task_id, $lead_id, 'json');
        $saveData = $save instanceof \Illuminate\Http\JsonResponse ? $save->getData(true) : $save;
        if (isset($saveData['url'])) {
            return ['success' => true, 'url' => $saveData['url']];
        } else {
            return $save; 
        }
    }
    public function job_card_approve(Request $request, $id)
    {
        ...
        if ($task->employee_id) {
            $data['employee'] = Admin::where('id', $task->employee_id)
                ->first();
        } else {
    ...
    {
        return view('admin-views.documents.approve_form', compact('id'));
    }
    public function rr_approve_submit(Request $request)
    {
        ...
        if ($receivableRec->employee_id) {
            $data['employee'] = Admin::where('id', $receivableRec->employee_id)
                ->first();
        } else {
            $vendor_id = $data['store']->vendor_id;
            $data['employee'] = Vendor::where('id', $vendor_id)->first();
    ...
    {
        $receipts = ReceivableReceipt::where('store_id', 0)->paginate(10);
        return view('admin-views.documents.recievable_reciepts_list', compact('receipts'));
    }
    public function job_cards_list(Request $request)
    {
        $job_cards = JobCard::where('store_id', 0)->paginate(10);
        return view('admin-views.documents.job_cards_list', compact('job_cards'));
    }
}
