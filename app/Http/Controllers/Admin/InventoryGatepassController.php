...
class InventoryGatepassController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->salesman_id == 'add_new') {
            ...
            $staff = Admin::where("id", $request->salesman_id)->first();
        ...
        $docData = Helpers::generateInventoryGatepass($invoice, $driver_data, $type, $request, 0);
    ...
    public function return(Request $request, $id)
    {
        ...
        $invoice = ManualInvoice::where("invoice_id", $gatepass->invoice_id)->where('vendor_id', 0)->latest('id')->first();
        $gp_items = InventoryGatepassItem::with('unitId')->where('gatepass_id', $gatepass->id)->get();
        // prx($invoice_items);
        return view('admin-views.inventory.gatepass.return', compact('gatepass', 'gp_items', 'invoice', 'store'));
    }
    public function gatepass_list(Request $request, $tab)
    {
        ...
        switch ($tab) {
            ...
            case 'purchase':
                $gatepass = InventoryGatepass::with('invoice')->where('type', 'purchase')->where('store_id', 0)->get();
                break;
            case 'purchase-return':
                $type = 'return-gp';
                $gatepass = InventoryGatepass::with('invoice', 'gpItems')->where('type', 'purchase')->where('returned', 1)->where('store_id', 0)->get();
                break;
            case 'sale':
                $gatepass = InventoryGatepass::with('invoice')->where('type', 'sale')->where('store_id', 0)->get();
                break;
            case 'sale-return':
                $type = 'return-gp';
                $gatepass = InventoryGatepass::with('invoice', 'gpItems')->where('type', 'sale')->where('returned', 1)->where('store_id', 0)->get();
                break;
            default:
                $gatepass = collect(); // safe fallback
                break;
        }
        return view('admin-views.inventory.gatepass.purchase', compact('gatepass', 'type'));
    }
}
