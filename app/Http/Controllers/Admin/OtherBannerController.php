...
class OtherBannerController extends Controller
{
    ...
    function store_offer_banner(Request $request)
    {
        ...
        $banner->store_id = 0; // 0 for admin
        $banner->image = $image_name;
        $banner->start_date = $request->start_date;
        $banner->end_date = $request->end_date;
        $banner->title = $request->title;
        $banner->url = $request->url;
        $banner->zone = $request->zone_id == 'all' ? 0 : $request->zone_id;
        $banner->save();

        Toastr::success(translate('messages.banner_added_successfully'));
        return back();
    }
    function offer_banner_approve(Request $request, $id)
    {
        OfferBanner::where('id', $id)->update(['approved' => 1]);
        Toastr::success(translate('messages.banner_approved_successfully'));
        return back();
    }
    function offer_banner_delete(Request $request, $id)
    {
        OfferBanner::where('id', $id)->delete();
        Toastr::success(translate('messages.banner_deleted_successfully'));
        return back();
    }
    function offer_banner_status(Request $request, $id, $status)
    {
        OfferBanner::where('id', $id)->update(['status' => $status]);
        Toastr::success(translate('messages.banner_status_changed_successfully'));
        return back();
    }
