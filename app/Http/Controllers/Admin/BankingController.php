...
class BankingController extends Controller
{
    public function bank_account(Request $request)
    {
        ...
        $accounts = StoreBankAccount::where('store_id', 0)
            ->get();

        $account_last_txn =  StoreBankTransaction::where('store_id', 0)
            ->whereBetween('txn_date', [$fyStart, $fyEnd])
            ->latest('id') // tie-breaker if same date
            ->first();
        // prx($account_last_txn);
        return view('admin-views.account.banking.bank-account.index', compact('accounts', 'fyStart', 'fyEnd', 'year', 'account_last_txn'));
    }
    ...
    public function bank_account_detail(Request $request, $id)
    {
        ...
        $sales = Helpers::bank_transactions_calendar($id, 0);
        // prx($sales);
        ...
        return view('admin-views.account.banking.bank-account.detail', compact('account', 'transactions', 'data', 'sales', 'year', 'files'));
    }
    public function bank_account_detail_main(Request $request)
    {
        ...
        $accounts = StoreBankAccount::where('store_id', 0)->get();

        return view('admin-views.account.banking.bank-account.detail-main', compact(
            'transactions',
            'data',
            'sales',
            'year',
    ...
    {
        $transactions = StoreBankTransaction::with('bankAccount')->where('store_id', 0)->get();
        $headings = [
            'Sl',
            'Bank Name',
            'Bank Account Number',
    ...
    {
        $bank_account = StoreBankAccount::where(['id' => $id, 'store_id' => 0])->first();
    ...
    {
        $file = StoreBankTransactionFile::where(['id' => $id, 'store_id' => 0])->first();
        if ($file) {
            $file->delete();
