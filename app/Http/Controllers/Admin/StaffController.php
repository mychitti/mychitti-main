...
class StaffController extends Controller
{
    public function save_settings(Request $request)
    {
        ...
        BusinessSetting::updateOrInsert(
            ['key' => 'emp_prefix_status'],
            ['value' => $request->has('prefix_status') ? 1 : 0]
        );
        BusinessSetting::updateOrInsert(
            ['key' => 'emp_prefix'], 
            ['value' => $request->has('prefix') ? $request->prefix : '']
        );
    ...
    public function teams()
    {
        ...
        $teams = StoreTeam::with('members')->where('store_id', 0)->paginate(10);
        return view('admin-views.employee.teams', compact('teams', 'staff'));
    }
    public function team_edit(Request $request, $id)
    {
        ...
        $team_members = StoreTeamMember::with('adminEmployee')->where('team_id', $id)->get();
        // prx($team_members);
        return view('admin-views.employee.teams_edit', compact('team', 'staff', 'team_members'));
    } 
    ...
    {
        $storeId = 0;
        $tAndCContentForStaff = DB::table('vendor_terms_conditions')->where('type', 'for_staff')->where('vendor_id', $storeId)->first();
        $prefix_status = BusinessSetting::where('key', 'emp_prefix_status')->first()?->value;
        $emp_prefix = BusinessSetting::where('key', 'emp_prefix')->first()?->value;
        return view('admin-views.staff.settings', compact('tAndCContentForStaff', 'prefix_status', 'emp_prefix'));
    }
    ...
    {
        $staff = Staff::find($id);
        $departments = Department::where('status', 1)->where('vendor_id', 0)->get();
    ...
    public function delete_department(Request $request, $id)
    {
        ...
        try {
            ...
            $staffCount = Admin::where('department_id', $department->id)->count();
