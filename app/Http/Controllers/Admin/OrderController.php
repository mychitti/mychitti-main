...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        if ($request->order_status == 'delivered') {
           ...
           Mail::to($order->customer->email)->send(new \App\Mail\CancelOrder($order->id));
        ...
        $order->save();
          ...
          // send mail and notification to vendor 

          $title = "Order Status Changed #" . $request->id;
          $msg = "Order status for order id " .  $request->id . ' is changed to '. Ucfirst($request->order_status) ;
          $acceptnce_id = '';
          $to = $order->store->id;
          $url = route('vendor.order.details', [$request->id]);
          $user_typ = 'vendor';
        ...
        //   prx($order->store->id);
          ...
          // end send mail and notification to vendor 


        if (!Helpers::send_order_status_notification($order)) {
    ...
    public function generate_order_invoice($id)
    {
        ...
        if ($order->invoice_pdf && Storage::disk('public')->exists('invoice/' . $order->invoice_pdf)) {
            Storage::disk('public')->delete('invoice/' . $order->invoice_pdf);
        }
        $user = User::find($order->user_id);
        if(!$user->pin_code){
            $cAddress = json_decode($order->delivery_address);
            $user->pin_code = getPincodeFromCoordinates( $cAddress->latitude,  $cAddress->longitude );
            $user->save();
        } 

        $store_pin_code = Store::find($order->store_id)->pin_code;
        $data['user_state_code'] = getStateCodeFromPincode($user->pin_code);
        $data['store_state_code'] = getStateCodeFromPincode($store_pin_code);
    ...
    $mpdf = new Mpdf([
        ...
        $data['billing_address'] = CustomerAddress::where('user_id', $user->id)->first();
        $data['user'] = $user;
        $html = View::make('invoice_template.shop', compact('order', 'data'))->render();
