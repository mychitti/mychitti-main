...
class InventoryPurchaseController extends Controller
{
    ...
    public function return(Request $request)
    {
        ...
        $return_slips = ReturnPurchaseSlip::where('store_id',0)->get();

        return view('admin-views.inventory.purchase.return', compact('return_slips','purchased_order_details', 'preset'));
    }
    public function return_store(Request $request)
    {
        ...
        $invoices = ManualInvoice::whereIn('invoice_id', $invoiceIds)->where('bill_to', 0)->where('bill_to_type', 'vendor')
    ...
    public function purchase_orders(Request $request)
    {
        ...
        $storeId = 0;
        $data['po_number_gst'] = Helpers::generatePoNumber('gst', 0)['po_number'];
        $data['po_number_nongst'] = Helpers::generatePoNumber('non-gst', 0)['po_number'];
        $vendors = StoreCustomer::where('store_id', $storeId)->where('user_type', 'vendor')->get();

        // low stock items 
        $low_stock_items = PurchaseOrder::with("inventoryItem")->where('store_id', 0)->get();

        // purchase orders 
        $purchase_orders = SupplyOrder::with(["order_items", 'invoice'])->where('store_id', 0)->whereBetween('created_at', [$formatted_from, $formatted_to])->get();

        return view('admin-views.inventory.purchase.orders', compact('data', 'preset', 'purchase_orders', 'low_stock_items', 'vendors'));
    }
    public function purchase_gatepass(Request $request)
    {
        return view('admin-views.inventory.gatepass.list', compact('data', 'purchase_orders', 'low_stock_items', 'vendors'));
    }
    ...
    public function export_purchase_orders(Request $request)
    {
        ...
        // purchase orders 
        $purchase_orders = SupplyOrder::with(["order_items", 'invoice'])->where('store_id', 0)->get();
