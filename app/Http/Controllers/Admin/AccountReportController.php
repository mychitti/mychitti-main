...
class AccountReportController extends Controller
{
    ...
    public function tax(Request $request, $export = null)
    {
        $storeId = 0;
        ...
        if ($export == 'export') {
            return Excel::download($this->exportTaxReport($invoices), 'tax_report.xlsx');
            _auditLogs('Exported tax report');
        }


        return view('admin-views.account.report.tax', compact('preset', 'serviceInvoices', 'invoices', 'saleInvoices', 'purchaseInvoices'));
    }
    public function exportTaxReport($invoices)
    {
        $headings = [
            'Sl',
            'Date',
    ...
    public function exportAuditLogsReport($audit_logs)
    {
        $headings = [
            'Sl',
            'Date',
        ...
        $rows = [];
        foreach ($audit_logs as $key => $e) {

            $rows[] = [
                $key + 1,
                $e['created_at'],
                $e['action'],
                $e['adminEmployee']?->f_name . ' ' . $e['adminEmployee']?->l_name
            ];
        }

        return new TaxReportExport($rows, $headings);
    }
    public function  audit_logs(Request $request, $export = null)
    {
        ...
        $audit_logs = AuditLog::with('adminEmployee')
        ...
        if ($export == 'export') {
            ...
            return Excel::download($this->exportAuditLogsReport($audit_logs), 'audit_logs_report.xlsx');
            _auditLogs('Exported tax report');
        }

        $audit_logs = $audit_logs->paginate(50);

        return view('admin-views.account.report.audit-logs', compact('preset', 'audit_logs'));
    }
}
