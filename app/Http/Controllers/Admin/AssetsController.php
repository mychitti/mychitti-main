...
class AssetsController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin-views.assets.index', compact('assets', 'categories', 'inventory_items', 'staff', 'asset_alotments'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $debit_account = Helpers::ensureFixedAssetAccount($request->category ?? $inventoryItem->item_name );
    ...
    {
        $assets = AssetAlotment::with('inventoryItem')->where('employee_id', auth('admin')->id())->where('store_id', 0)->paginate(10);
        // prx($assets);
        return view('admin-views.assets.alotted', compact('assets'));
    }
