...
// prx($request->item_id);
        ...
        $flash_sale->save();
        $data = [
            'title' => "Special Offer on Your Wishlist Pick",
            'description' => 'Get '.$item->name.' only at  ' .Helpers::currency_symbol() .$flash_sale->price.' Shop now and save big!',
            'order_id' => '',
            'image' => '',
            'type'=> 'block'
        ];
        $customers = DB::table('wishlists')->join('users', 'users.id', 'wishlists.user_id')->where('wishlists.item_id', $item->id)->get();
        foreach ($customers as $key => $value) {
            $fcm_token = $value->cm_firebase_token;
