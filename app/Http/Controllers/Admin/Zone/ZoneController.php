...
class ZoneController extends BaseController
{
    ...
    public function requests(){  
        $area_requests =   DB::table('zone_requests')
        ->leftJoin('users', 'users.id', '=', 'zone_requests.user_id')
        ->where('zone_requests.approved', 0)
        ->select('zone_requests.*','zone_requests.created_at as requested_at', 'zone_requests.id as req_id', 'users.*') // Add other columns as needed
        ->get();
        // print_r($area_requests);
        // die; 
        return view('admin-views.zone.requested_area', compact('area_requests'));
    } 
    ...
    public function approve(Request $request){
       ...
       ZoneRequest::find($request->id)->update(['approved'=> 1]);
