...
class VendorController extends Controller
{
    ...
    public function assignManualTrial(Request $request)
    {
        ...
        ManualTrial::create([
            'store_id' => $request->user_id,
            'submodule_id' => $request->plan_id,
    ...
    public function save_modules(Request $request)
    {
        $storeType =  StoreType::find($request->business_type_id);
        $storeType->permitted_submodules = $request->edit_sub_modules ? implode(',', $request->edit_sub_modules) : null;
        $storeType->save();
    ...
    public function type_store(Request $request)
    {
        ...
        $storeType->module_id = Config::get('module.current_module_id');
        $storeType->permitted_submodules = $request->edit_sub_modules ? implode(',', $request->sub_modules) : null;
        $storeType->save();
    ...
    public function update_id(Request $request)
    {
        ...
        $rules = [
            'id_number' => 'required|max:191',
            'otp_input' => 'required|max:4',
        ];
        ...
        $store->id_doc = $request->has('id_doc') ? Helpers::update('store/docs/', $store->id_doc, 'png', $request->file('id_doc')) : $store->id_doc;
    ...
    public function store(Request $request)
    {
        ...
        //     $rulesArr['gst_num']  = 'required';
        // }
        // if ($request->business_type == 'business') {
        $rulesArr['google_verification']  = 'required';
        // }
        // if (!$request->has('gst_number') || $request->gst_number == '') {
        //     $rulesArr['id_doc']  = 'required|mimes:png,jpg,jpeg,pdf';
        //     $rulesArr['id_number']  = 'required';
        // }

        if (Config::get('module.current_module_id') == 5) {
            $rulesArr['minimum_delivery_time'] = 'required';
            $rulesArr['maximum_delivery_time'] = 'required';
            $rulesArr['delivery_time_type'] = 'required';
        }
        ...
        // $vendor->email =   $randomEmail;
        // $vendor->phone = $randomPhone;
        ...
        // $store->phone = $randomPhone; 
        // $store->email =   $randomEmail;
        ...
        if ((isset($request->confirmation) && $request->confirmation == 'on')) {
            $store->gst_doc = null;
            $store->salary_below = 20;
        }
        if ((!$request->has('gst_number') || $request->gst_number == '') && $request->has('id_doc')) {
        ...
        try {
            ...
            $bankAccount->user_id = $store->id;
            $bankAccount->account_number =  $request->payment_method == 'bank' ?  $request->account_number : '';
            $bankAccount->payment_type = $request->payment_method;
            $bankAccount->account_holder_name =  $request->payment_method == 'bank' ?  $request->acc_holder_name : '';
            $bankAccount->ifsc_code =  $request->payment_method == 'bank' ?  $request->ifsc : '';
            $bankAccount->upi_id = $request->payment_method == 'upi' ? $request->upi_id  : '';
            $images = [];

            if (!empty($request->file('documents'))) {
                foreach ($request->documents as $file) {
                    $extension = $file->getClientOriginalExtension();
                    $image_name = Helpers::upload('vendor/documents/', $extension, $file);
                    $images[] = $image_name;
                }
            }
            $bankAccount->documents = json_encode($images);
            $bankAccount->save();
    ...
    public function buy_plan_for_store(Request $request)
    {
        ...
        $this->buyPlan($request->v_id, $request->post('plan_select'), $request->billing, $request->post('invoice_date'));
        Toastr::success(translate('messages.added_successfully'));
        return redirect()->back();
    }

    public function store_enabled_modules(Request $request, $storeId)
    {
        $store =  Store::withoutGlobalScopes()->where("id", $storeId)->first();
        $subscriptions = VendorSubscription::with('plan')->where('vendor_id', $storeId)
            ->where('plan_expiry', '>', now())->get();
        $history = VendorSubscription::with('plan')->where('vendor_id', $storeId)
            ->where('plan_expiry', '<', now())->get();
    ...
    public function get_matches(Request $request)
    {
        ...
        $customers = Store::withoutGlobalScopes()->where(function ($q) use ($query) {
    ...
    public function buyModule(Request $request)
    {
        ...
        foreach ($selectedModules as $item) {
            ...
            $subscription = new VendorSubscription;
            $subscription->vendor_id = $request->store_id;
    ...
    public function buyPlan($vendorId = NULL, $planId = NULL, $billing = true, $invoiceDate = null)
    {
        // Request $request, 
        if ($vendorId == NULL && $planId == NULL) {
            $vendorId = $request->post('v_id');
            $planId = $request->post('plan_select');
            $zone_id = DB::table('stores')->where('vendor_id', $vendorId)->first()->zone_id ?? null;
            $module_id = DB::table('stores')->where('vendor_id', $vendorId)->first()->module_id ?? null;
        }
        ...
        if ($variation) {
            ...
            if (!empty($planDetails->duration_count) && !empty($planDetails->duration_type)) {
                $interval = '+' . $planDetails->duration_count . ' ' . $planDetails->duration_type;
                $expDate->modify($interval);
            } else {
                $planDetails->duration_count =  0;
                $planDetails->duration_type = 'day';
                $interval = '+' . $planDetails->duration_count . ' ' . $planDetails->duration_type;
                $expDate->modify($interval);
            }
        ...
        $plan_amount = (isset($variationData) && $variationData) ? $variationData['price'] : $planDetails->price;
        $plan_discount = (isset($variationData) && $variationData) ? $variationData['discount'] : $planDetails->discount;
        ...
        if ($billing) {
            ...
            $invoice->invoice_serial = BusinessSetting::where('key', 'admin_bill_serial_number')->first()->value - 1;
            $invoice->vendor_id = NULL;
            $invoice->bill_to = $vendorId ?? null;
            $invoice->bill_to_type = 'vendor';
            $invoice->module_id = $module_id ?? 6;
            $invoice->total_amount =  round(_taxIncludedPrice(_discountedPrice($plan_amount, $plan_discount, 'percent'), $planDetails->gst_percent, 'actual'));
            $invoice->payment_method = 'Online';
            $invoice->tax_type =  'gst';
            ...
            $InvoiceItem->qty = 1;
            $InvoiceItem->price =  _discountedPrice($plan_amount, $plan_discount, 'percent');
    ...
    function updateStoreItems($store, $services, $action)
    {
        if (!empty($services)) {
            foreach ($services as $value) {
                ...
                if ($action === 'remove') {
                    ...
                    // Add store ID and prevent duplicates
                    $storeArr[] = (string)$store->id;
    ...
    public function update(Request $request, Store $store)
    {
        ...
        // You can then use the $validator object to check for validation failures and handle accordingly
        if ($validator->fails()) {
            // Handle the validation errors
            $errors = $validator->errors();
            // return or process the errors as needed
        }
        ...
        //account details
        $bankAccount =  AccountDetail::where('user_type', 'vendor')->where('user_id', $store->id)->first();
        if (!$bankAccount) {
            $bankAccount = new AccountDetail();
            $bankAccount->user_type = 'vendor';
            $bankAccount->user_id = $store->id;
            ...
            $images = json_decode($bankAccount->documents);
        }
        $bankAccount->account_number =  $request->payment_method == 'bank' ?  $request->account_number : '';
        $bankAccount->payment_type = $request->payment_method;
        $bankAccount->account_holder_name =  $request->payment_method == 'bank' ?  $request->acc_holder_name : '';
        ...
        // && $request->file('gst_doc') != null
        if ((!isset($request->confirmation) || $request->confirmation == '' || Config::get('module.current_module_id') == 5)) {
            if ($request->file('gst_doc') != null) {
                $extension = $request->file('gst_doc')->getClientOriginalExtension();
                $store->gst_doc = Helpers::upload('store/docs/', $extension, $request->file('gst_doc'));
            }
            $store->gst_number = $request->gst_num;
    ...
    public function terms_and_conditions_store(Request $request)
    {
        if (Config::get('module.current_module_id') == 5) {
            ...
            BusinessSetting::where('key', 'shop_vendor_tnc')->update([
                'value' => $request->terms_and_conditions
            ]);
        } elseif (Config::get('module.current_module_id') == 6) {

            BusinessSetting::where('key', 'service_vendor_tnc')->update([
                'value' => $request->terms_and_conditions
            ]);
        }
    ...
    public function terms_and_conditions(Request $request)
    {
        ...
        return view('admin-views.vendor.terms-and-conditions', compact('terms_and_conditions'));
    }
    public function view(Request $request, $store_id, $tab = null, $sub_tab = 'cash')
    {
        ...
        if ($tab == 'account') {
            return view('admin-views.vendor.view.account_details', compact('store'));
        } elseif ($tab == 'documents') {
            StoreDocument::where('checked', 0)
                ->update(['checked' => 1]);

            $store_documents = StoreDocument::where('store_id', $store->id)->get();
            return view('admin-views.vendor.view.documents', compact('store', 'store_documents'));
        } elseif ($tab == 'actions') {
            return view('admin-views.vendor.view.action', compact('store'));
        ...
        if ($store->category_1) {
            ...
            $det = Category::find($store->category_1);
            if ($det) {
                array_push($categories, $det->name);
            }
        }
        if ($store->category_2) {
            $c = Category::find($store->category_2);
            if ($c) {
                array_push($categories, $c->name);
            }
        }
        if ($store->subcategories) {
            foreach (explode(',', $store->subcategories) as  $subcat) {
                $s = Category::find($subcat);
                if ($s) {
    ...
    public function verify_doc(Request $request, $id)
    {
        ...
        if (Schema::hasColumn('stores', $type)) {
            ...
            ActionLog::create([
                ...
                'user_type' => 'admin',
                'action' => 'verified',
                'model_type' => 'store', // e.g. App\Models\Category
                'model_id' => $store->id,
                'description' => 'Verified vendor document ' . $type . ' for store ID ' . $store->id,
                'created_at' => now(),
            ]);
        } else {
            // Toastr::warning("Column '$type' does not exist in stores table");
        }
    ...
    public function suspend_account(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            foreach ($request->file('file') as $img) {
                ...
                $doc = Helpers::upload('store/docs/', $extension, $img);
                array_push($img_names, $doc);
            }
            $docs = json_encode($img_names);
        } else {
            $docs = json_encode([]);
        }

        $store = Store::with('vendor')->find($request->store_id);

        if ($store) {
            $store->suspended = 1;
            $store->status = 0;
            $store->suspension_docs = $docs;
            $store->suspension_reason = $request->reason;
            $store->status_updated_by = auth('admin')->user()->id;
            $store->save();

            if ($store->vendor) {
                $store->vendor->suspended = 1;
                // $store->vendor->status = 0;
                $store->vendor->save();
            }
        }
        Toastr::success("Suspended store successfully");
    ...
    public function report(Request $request)
    {
        ...
        // $stores->whereBetween('updated_at', [$from .' 00:00:00', $to .' 23:59:59']);
        // ->whereNotNull('status_updated_by')
        return view('admin-views.vendor.report', compact('stores'));
    }
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($request->button == 'import') {
            ...
            foreach ($collections as $key => $collection) {
                if (
                    $collection['ownerFirstName'] === "" || $collection['storeName'] === "" || $collection['phone'] === ""
        ...
        foreach ($collections as $key => $collection) {
            if (
                $collection['id'] === "" || $collection['ownerId'] === "" || $collection['ownerFirstName'] === "" || $collection['storeName'] === "" || $collection['phone'] === ""
                || $collection['email'] === "" || $collection['latitude'] === "" || $collection['longitude'] === ""
                || $collection['zone_id'] === "" ||  $collection['DeliveryTime'] === ""  || $collection['Tax'] === "" || $collection['logo'] === ""
            ) {
                Toastr::error(translate('messages.please_fill_all_required_fields'));
